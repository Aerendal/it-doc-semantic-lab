...
Ogólne problemy systemowe (OS/app/infra) i standardowe remedia.
...
- [ ] Health checks i restart policy.
- [ ] Monitoring CPU/RAM/IO, alerty.
- [ ] Log rotation i remote shipping.
- [ ] IaC/config-as-code, parity środowisk.
- [ ] Circuit breaker/timeout/retry ustawione.
...
- **Wysokie CPU/RAM**: memory leak, runaway process → profiling, limitowanie, restart policy, autoscaling.
- **I/O bottleneck**: brak indeksów, małe blokady → indeksy, batch, cache, tune FS/IO scheduler.
- **Zawieszanie/usługi niedostępne**: brak health checks → liveness/readiness, auto-restart, circuit breaker.
- **Konfiguracja**: różne środowiska → IaC, feature flags, config-as-code.
- **Logi zapełniają dysk**: brak rotacji → log rotation, remote shipping, quota.
- **DNS/proxy**: błędy → fallback resolver, cache, health na DNS/HTTP.
