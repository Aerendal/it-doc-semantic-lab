---
title: Incident Response for Customers
...
Opisać, jak obsługiwać zgłoszenia incydentów od klientów: przyjęcie, triage, komunikacja, eskalacja, rozwiązanie, postmortem i feedback.


## Zakres i granice

- Obejmuje: kanały zgłoszeń (CS/portal/API), klasyfikację/severity, triage, eskalacje do zespołów technicznych/IR, komunikację z klientem, SLA odpowiedzi/rozwiązania, action log, raportowanie i postmortem.
- Poza zakresem: wewnętrzne działania techniczne (runbooki IR/SRE) oraz zgłoszenia regulacyjne (opisane w Regulatory Incident Reporting).
...
- Kanały zgłoszeń i klasyfikacja/severity
- Triage i eskalacje (kto, kiedy, gdzie)
- Komunikacja z klientem (szablony, częstotliwość)
- SLA odpowiedzi/rozwiązania i monitorowanie
- Action log, statusy i raportowanie
- Postmortem/feedback i doskonalenie
...
- Ustal severity i kanał komunikacji; prowadź action log i SLA; sekcje N/A uzasadnij.
- Dodaj quick-links i checklisty DoR/DoD w `reports/checklist_atomic.jsonl`.
- Po incydencie uzupełnij feedback/postmortem z perspektywy klienta.
...
- Zgłoszenia klientów (ticket/portal/API), status systemów, polityka IR, matryca severity.
- Lista kontaktów/ownerów, kanały komunikacji, szablony odpowiedzi.


## Wyjścia

- Obsłużone zgłoszenia z komunikacją, action logiem i statusami SLA.
- Eskalacje do zespołów technicznych; raporty dla CS/biznesu.
- Postmortem/lessons learned z perspektywy klienta.
...
- status_comms_playbook.md (jeśli istnieje)
- logging_and_audit_trail.md


## Wymagane rozwinięcia / streszczenia

- Tabela: severity → SLA response/resolve → kanał → szablon → owner.
- Action log i wzór odpowiedzi.


## Wymagane powiązania

- Polityka IR, matryca severity, kanały CS/CRM, szablony, raporty statusu.


## Kryteria DoR

- [ ] Kanały i szablony gotowe; matryca severity dostępna.
- [ ] Role/ownerzy i SLA uzgodnione.
- [ ] Narzędzia ticketing/CRM dostępne.


## Kryteria DoD

- [ ] Zgłoszenia obsłużone zgodnie z SLA; komunikacja i action log zapisane.
- [ ] Eskalacje/rozwiązania udokumentowane; postmortem/feedback wpisany.
- [ ] Quick-links/checklisty zaktualizowane, metadane bieżące.


## Artefakty do załączenia

- Action log/ticket, komunikaty do klienta.
- Raport SLA i status rozwiązania.
- Postmortem/feedback klienta.


## Walidacja / testy

- Dry-run komunikacji na scenariuszu klienta.
- Sprawdzenie SLA w narzędziu ticketowym.


## Metryki monitorowane

- Odpowiedź/rozwiązanie vs SLA; CSAT/NPS klienta.
- Liczba zgłoszeń i eskalacji; powtarzalność problemów.
- Czas do pierwszej odpowiedzi; liczba przeprosin/rewizji komunikatu.


## Utrzymanie i aktualizacje

- Przegląd po incydentach i cyklicznie (np. miesięcznie); aktualizuj szablony/kanały/SLA.
- Synchronizuj z Communication Plan i Regulatory Incident Reporting.


## Zakończenie

Po spełnieniu DoD podlinkuj artefakty, odhacz checklisty w `reports/checklist_atomic.jsonl` i przekaż wnioski zespołom CS/IR.
