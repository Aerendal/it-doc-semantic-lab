---
title: Migration Status Updates
...
## Cel dokumentu
Ustalić, jak komunikować postęp migracji (danych/usług) w trakcie okna migracyjnego: kto, jak często, jakim kanałem i z jaką treścią raportuje status, ryzyka i ewentualne rollbacki.

## Zakres i granice
Obejmuje: odbiorców, kanały, cadence update’ów, format statusu (planowane/wykonane/problemy), kryteria rollback/hold, log komunikacji. Nie obejmuje: technicznych kroków migracji (są w Migration Plan/Runbook).

## Wejścia i wyjścia
- Wejścia: Migration Plan, lista kroków i checkpointów, okno migracyjne, ryzyka, zależności systemów, polityki komunikacji/PR.
- Wyjścia: harmonogram statusów, szablony komunikatów, log update’ów, lista ownerów i kanałów.
...
- Odbiorcy i kanały
- Cadence i checkpointy (start, cutover, walidacja, zakończenie, rollback trigger)
- Format komunikatu (co planowane, co wykonane, metryki walidacji, problemy/ryzyka, next)
- Eskalacja i komunikacja w razie blokera/rollback
- Dziennik statusów i odpowiedzialni
