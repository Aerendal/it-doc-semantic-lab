---
title: Service Level Philosophy
...
Service Level Philosophy — szablon dokumentu IT.

Zawiera krok po kroku instrukcje dla operatorów IT do wykonania powtarzalnych zadań lub reagowania na zdarzenia. Każdy krok powinien: być atomowy, zawierać oczekiwany rezultat, wskazywać akcję przy błędzie i określać czas wykonania.

> Dokument satelitarny do: "Service Level Monitoring" - rozszerza lub uszczegolawia jego tresc.
...
- **Wejścia** (co musi być dostępne przed wypełnieniem): Architektura systemu, topologia sieci, dane dostępu (vault/secrets), poprzednie incydenty (lessons learned), wymagania SLA.
- **Wyjścia** (co dokument wytwarza jako rezultat): Udokumentowana procedura gotowa do wykonania, lista wymaganych uprawnień, kryteria sukcesu/niepowodzenia, log wykonania.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): Log operacji (audit trail), metryki dostępności systemu, raporty z testów DR, baza wiedzy.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Architektura systemu, CMDB, polityki bezpieczeństwa (zasada najmniejszych uprawnień), wymagania SLA/OLA.
