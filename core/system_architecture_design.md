...
Opisać architekturę bezpiecznego systemu zgodnie z wymaganiami OWASP ASVS V1 (Architecture, Design and Threat Modeling) — definiując zasady security-by-design na poziomie systemu. Dokument obejmuje: model zagrożeń (STRIDE/DREAD), security boundaries, trust levels, data flow diagram z oznaczonym trust boundary, wybór mechanizmów bezpieczeństwa (authn, authz, kryptografia, input validation) i dowody spełnienia wymagań ASVS.
...
2. Widoki: logiczny/proces/deployment.
3. Dane i integracje.
4. Niefunkcjonalne: SLO, bezpieczeństwo.
...
- [ ] Wymagania i widoki opisane.
- [ ] Dane/integracje zmapowane.
- [ ] NFR i decyzje udokumentowane.
- [ ] Plan wdrożenia istnieje.
