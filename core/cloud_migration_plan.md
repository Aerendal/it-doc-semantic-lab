...
## Cel dokumentu
Krótka definicja: Cloud Migration Plan opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
1. Zakres migracji: systemy/usługi, fale, priorytety, model chmury (IaaS/PaaS/SaaS), regiony.
2. Ocena i przygotowanie: inwentaryzacja, zależności, kompatybilność, readiness (security/compliance), dekompozycja na migracyjne strumienie.
3. Architektura docelowa i kontrolki bezpieczeństwa: IAM, sieć/segmentacja, szyfrowanie, logging/monitoring, CI/CD, DR/BCP, compliance (ISO/SOC/PCI/RODO).
4. Plan danych: migracja danych, etapy cutover, walidacja integralności, maskowanie/PII, backup/rollback.
5. Plan aplikacji: refactor/rehost/replatform, kontenery/serverless, feature flags, testy przed/po, smoke/rollback.
6. Operacje i SRE: SLO/SLA, monitoring, alerty, runbooki, incident readiness, limit/quoty, koszt (FinOps).
7. Harmonogram i fale: kamienie milowe, okna zmian, sekwencje, freeze, gotowość biznes/support.
8. Testy i weryfikacja: performance/load, bezpieczeństwo (skany/pentest), DR test, cutover rehearsal.
9. Ryzyka i plan awaryjny: scenariusze, mitigacje, decyzje go/no-go, fallback on-prem/hybrid.
10. Komunikacja i zgody: stakeholderzy, CAB, klienci/regulator (jeśli dotyczy), status page/PR.

## Wymagane rozwinięcia
- Architektura/controlki rozwijają: Cloud Security Baseline, Cloud Infrastructure Requirements, IAM/Network/SIEM runbooks.
- Plan danych rozwija: Data Migration Plan, Data Classification/PII Handling, Backup/DR.
- Testy/weryfikacja rozwijają: Performance Test Plan, Security Test Plan (skany/pentest), DR drills.

## Wymagane streszczenia
- Streszczenie fal migracji, kryteriów go/no-go, rollbacku i wymagań compliance/security.
...
- DoR:
  - [ ] Zakres systemów/fal i regionów określony; readiness (security/compliance) oceniony.
  - [ ] Docelowa architektura i kontrolki (IAM/sieć/szyfrowanie/logi/DR) uzgodnione; plany danych i aplikacji wstępnie opisane.
  - [ ] Role/ownerzy i okna zmian/freeze ustalone; dane/PII/backup/rollback wymagania zebrane.
- DoD:
  - [ ] Plan migracji opisuje fale, harmonogram, kontrolki bezpieczeństwa, plany danych/aplikacji, testy i DR.
  - [ ] Kryteria go/no-go i rollback zdefiniowane; testy (perf/security/DR) zaplanowane lub wykonane.
  - [ ] Ryzyka/mitigacje, komunikacja i zgody ujęte; dokument wersjonowany i udostępniony.
