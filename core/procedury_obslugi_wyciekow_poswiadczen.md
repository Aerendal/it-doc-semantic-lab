---
title: Procedury obsługi wycieków poświadczeń
...
aligned_by: codex
---
# Procedury obsługi wycieków poświadczeń
...
Zapewnić szybkie wykrycie i reakcję na wycieki poświadczeń (klucze/hasła/tokeny), by ograniczyć ryzyko kompromitacji.
...
1. Detekcja: źródła (DLP, repo scan, SIEM), klasyfikacja (public/internal), typ sekretu.
2. Izolacja i rotacja: natychmiastowe unieważnienie/rotacja kluczy, blokada kont, revoke tokens.
3. Analiza wpływu: logi użycia, zakres ekspozycji, zasięg (prod/stage), timeline.
4. Remediacja: usunięcie sekretu z repo/logów, update konfiguracji, poprawki pipeline (secret management).
5. Komunikacja i zgłoszenia: incident response, właściciele systemów, klient/regulator (jeśli wymagane).
6. Postmortem: przyczyna wycieku, działania trwałe (vault, scanning, lint), szkolenia.
...
- [ ] Sekret unieważniony/rotowany natychmiast; użycie sprawdzone.
- [ ] Sekret usunięty z repo/logów; pipeline/secret management poprawione.
- [ ] Komunikacja/zgłoszenia wykonane wg polityk; audyt zapisany.
- [ ] Postmortem i działania trwałe (scan, vault, trening) wdrożone.
