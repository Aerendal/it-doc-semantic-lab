...
Opisać techniczny projekt rozwiązania/komponentu: architektura, dane, interfejsy, NFR, ryzyka i plan testów.


## Zakres i granice

- Obejmuje: kontekst i wymagania, architekturę logiczną/fizyczną, dane i modele, interfejsy/API, przepływy, NFR (wydajność, bezpieczeństwo, dostępność), decyzje/alternatywy, migrację, testy i rollout.
- Poza zakresem: pełny kod/implementacja, backlog zadań (mapowane linkami).
...
## Wejścia i wyjścia
- Wejścia: FRS/NFR, ADR/RFC, ograniczenia architektoniczne, standardy org, wyniki spike’ów/prototypów.
- Wyjścia: zatwierdzony projekt techniczny, lista otwartych ryzyk/pytań, wymagania do testów niefunkcjonalnych, dane do planu migracji i wdrożenia.
## Założenia
...
- Konsumuje: ADR, System Design, Security/Privacy requirements, Data governance rules.
- Dostarcza do: Implementation Plan, Test Strategy/Performance Plan, Runbook, Deployment Plan.
## Fazy cyklu życia
...
- Architektura i diagramy (logic/fizyczna)
- Dane/model i magazyny
- Interfejsy/API i kontrakty
- NFR i SLO/SLA
- Bezpieczeństwo/zgodność
- Migracja i deployment/rollback
- Testy i walidacja
...
- ADR, Solution Guide, Security, Data Governance, Release/Change Mgmt, Performance Requirements.
...
- Wymagania (biz/tech), ADR, architektura referencyjna, ograniczenia, dane wejściowe, SLO/SLA.


## Wyjścia

- Spójny projekt techniczny z diagramami, kontraktami API, planem testów i ryzykami.



## Jak używać (checklista)

- Wypełnij wymagania i kontekst; dodaj diagramy.
- Opisz dane i API; zdefiniuj NFR/SLO i bezpieczeństwo.
- Przygotuj plan migracji/testów i rollout/rollback.
- Zapisz ryzyka i decyzje; podlinkuj ADR/backlog.


## Wymagane rozwinięcia / powiązania

- Diagramy (C4), spec API, tabela NFR, plan testów, plan migracji, risk log.


## Kryteria DoR

- Wymagania i ADR dostępne; SLO znane; interesariusze zidentyfikowani.


## Kryteria DoD

- Sekcje wypełnione, diagramy/API/NFR/testy gotowe; przegląd i akceptacja architektoniczna.


## Artefakty

- TDD, diagramy, spec API, plan testów/migracji, risk log.


## Walidacja

- Przegląd architektoniczny/secu; testy POC; zgodność z NFR/SLO; plan rollback.


## Metryki

- Czas przeglądu/akceptacji, liczba iteracji TDD, zgodność implementacji z TDD.


## Utrzymanie

- Aktualizacja TDD przy zmianach zakresu/architektury; wersjonowanie; retrospektywa po wdrożeniu.


## Zakończenie

TDD zapewnia wspólne zrozumienie projektu; utrzymuj je z diagramami, NFR i ryzykami.
