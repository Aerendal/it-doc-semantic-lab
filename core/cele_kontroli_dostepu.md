...
Zdefiniować cele i KPI dla kontroli dostępu: bezpieczeństwo (least privilege), zgodność, użyteczność i audytowalność.


## Zakres i granice

- Obejmuje: modele uprawnień, czas nadawania/odbierania, recertyfikacje, zgodność (SoD), incydenty dostępu, doświadczenie użytkownika.
- Nie obejmuje: szczegółowej konfiguracji IdP/SSO (oddzielny dokument).
...
1. Cele/KPI (np. czas provision/deprovision, % recertyfikacji na czas, brak naruszeń SoD).
2. Zakres systemów i ról.
3. Procesy (nadawanie, zmiana, odbieranie, recertyfikacja).
4. Zgodność i audyt (SoD, logi, raporty).
5. UX i samoobsługa (request, approval, status).
...
- [ ] Procesy access lifecycle opisane.
- [ ] SoD i audyt pokryte.
- [ ] UX/samoobsługa dostępna.
