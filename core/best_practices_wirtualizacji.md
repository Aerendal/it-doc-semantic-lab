---
title: Best practices wirtualizacji
...
Zebrać dobre praktyki zarządzania środowiskami wirtualnymi (hypervisor/KVM/VMware/Proxmox): wydajność, bezpieczeństwo, HA i operacje.


## Zakres i granice

- Obejmuje: sizing VM, CPU/RAM/NUMA, storage (thin/thick, alignment), sieć (vSwitch/VLAN/MTU), izolację, snapshot/backup, HA/DR, monitoring.
- Nie obejmuje: konteneryzacji (oddzielny dokument) ani polityk kosztowych.
...
1. Sizing i alokacja zasobów (CPU pinning/NUMA, RAM ballooning, overcommit).
2. Storage i IO (typy dysków, cache, alignment, backup/snapshot).
3. Sieć (vSwitch/VLAN, MTU, SR-IOV passthrough).
4. Bezpieczeństwo (izolacja, aktualizacje, hardening hypervisor).
5. HA/DR (cluster, live migration, quorum, testy).
6. Monitoring i capacity (metryki, alerty).
...
- [ ] Sizing i overcommit uzasadnione.
- [ ] Storage i snapshot/backup skonfigurowane.
- [ ] Sieć/MTU/VLAN opisane; SR-IOV gdy potrzeba.
- [ ] HA/DR przetestowane.
- [ ] Monitoring i capacity działa.
