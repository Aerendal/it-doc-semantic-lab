...
Zdefiniować procedury reagowania na incydenty w pipeline’ach danych/przetwarzania, by zminimalizować przerwy i utratę jakości danych.
...
1. Detekcja: źródła alertów (DQ, lag, failures, DLQ), klasyfikacja incydentów.
2. Triage: zakres (job/źródło/konsument), priorytet, wpływ na SLA/klientów.
3. Reakcja: retry/replay, przełączenie na backup, izolacja, wstrzymanie ingest, komunikacja.
4. Przyczyny i RCA: logi, trace, schematy, zmiany ostatnie, dependency map.
5. Odtworzenie i weryfikacja: reprocessing, walidacje, korekty danych, publish status.
6. Post-incident: postmortem, akcje trwałe, aktualizacja runbooków/alertów.
...
- [ ] Klasyfikacja incydentów i priorytety zdefiniowane.
- [ ] Runbook reakcji (retry/replay/backup) opisany; komunikacja ustawiona.
- [ ] Walidacja danych po naprawie wykonana.
- [ ] Postmortem i poprawki alertów/runbooków przeprowadzone.
