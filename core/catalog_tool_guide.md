...
Przewodnik użycia narzędzia katalogowego (np. data catalog/service catalog): jak szukać, dodawać i utrzymywać wpisy, role i procesy jakości.


## Zakres i granice

- Obejmuje: wyszukiwanie i tagowanie, dodawanie zasobów (schematy, usługi, API), właściciele i stewardzi, recenzje/approval, wersjonowanie metadanych, integracje (lineage, glossary), uprawnienia.
- Nie obejmuje: polityk danych/retencji (osobne dokumenty) ani instalacji narzędzia.
...
1. Zakres zasobów w katalogu (dane/usługi/API).
2. Dodawanie/edycja wpisów (wymagane pola, szablony, właściciel).
3. Tagowanie i słownik pojęć (glossary, kategorie).
4. Jakość i przeglądy (approval, freshness, SLA metadanych).
5. Integracje (lineage, CI z repozytorium schematów).
6. Dostępy i role (viewer/editor/steward/admin).
...
- [ ] Wpisy mają ownera, tagi i komplet wymaganych pól.
- [ ] Glossary i kategorie używane spójnie.
- [ ] Przeglądy/approval i świeżość metadanych zaplanowane.
- [ ] Dostępy zgodne z rolami (least privilege).
