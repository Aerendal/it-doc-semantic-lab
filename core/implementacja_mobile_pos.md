---
title: Implementacja mobile POS
...
Wdrożyć mobilne stanowiska POS (smartfony/tablety) do sprzedaży i obsługi klienta z pełnym bezpieczeństwem i integracjami.
...
1. Zakres: typy transakcji, kanały (in-store, queue-busting, eventy), urządzenia.
2. Integracje: płatności (SDK/terminal softPOS), ERP/OMS/WMS, lojalność, e-paragony.
3. UX i offline: flow kasowe, skanowanie, zwroty, tryb offline/async sync.
4. Bezpieczeństwo: MDM, hardening urządzeń, szyfrowanie, tokenizacja kart, logowanie.
5. Operacje: zarządzanie urządzeniami, aktualizacje, helpdesk, rotacja kluczy.
6. Compliance: PCI DSS, fiskalizacja (jeśli dotyczy), zgody klienta.
...
- [ ] Integracje płatności/ERP/OMS/WMS przetestowane end-to-end.
- [ ] Flow sprzedaży/zwrotów i tryb offline działają.
- [ ] MDM/hardening i zabezpieczenia kart wdrożone; rotacja kluczy ustawiona.
- [ ] Wymagania compliance (PCI/fiskalizacja) spełnione i udokumentowane.
