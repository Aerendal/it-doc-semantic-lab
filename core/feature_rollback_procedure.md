---
title: Feature Rollback Procedure
...
## Cel dokumentu
Opisać, jak bezpiecznie wycofać konkretną funkcję (np. za pomocą feature flag/konfiguracji), minimalizując wpływ na użytkowników i dane.

## Zakres i granice
Obejmuje: warunki aktywacji rollback funkcji, kroki (flag off/config rollback), weryfikację po wyłączeniu, komunikację, log zmian. Nie obejmuje: pełnego rollbacku wersji aplikacji (ten jest w Rollback Plan/Procedure).

## Wejścia i wyjścia
- Wejścia: plan releasu, lista feature flags/konfiguracji, metryki wpływu, SLO/SLA, procedury monitoringu/alertów, wymogi komunikacji.
- Wyjścia: wyłączona funkcja, walidacja zdrowia systemu, log wykonania, komunikat do interesariuszy/klientów (jeśli dotyczy).
...
- Warunki wyzwolenia (metryki, błędy, feedback)
- Krok 1: Flaga/konfiguracja (kto, gdzie, jak)
- Krok 2: Walidacja po wyłączeniu (metryki, logi, UX)
- Krok 3: Komunikacja (wew./zew.) i aktualizacja statusu
- Krok 4: Sprzątanie/cleanup (usunięcie rolloutów, zadłużenie kodowe)
- Log działań i odpowiedzialni
