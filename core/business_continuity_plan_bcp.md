...
## Cel dokumentu
Zaplanować utrzymanie ciągłości kluczowych procesów biznesowych przy znaczących zakłóceniach (infra, ludzie, dostawcy, lokalizacje). Plan określa role, ścieżki decyzyjne, RTO/RPO procesów oraz zależności do DRP/IR, żeby ograniczyć czas przestoju i chaos komunikacyjny.

## Zakres i granice
- Obejmuje: krytyczne procesy i ich właścicieli, zależności do systemów/dostawców, scenariusze zakłóceń, RTO/RPO procesów, workarounds, komunikację kryzysową, testy i przeglądy BCP.
- Nie obejmuje: szczegółowych procedur DR IT (są w DRP) ani ewakuacji fizycznej/HSSE.

## Wejścia i wyjścia
- Wejścia: BIA, katalog procesów/właścicieli, zależności systemowe/dostawców, SLA, dane kontaktowe kryzysowe, proponowane RTO/RPO z IT/DRP, lokalizacje alternatywne.
- Wyjścia: BCP z priorytetami procesów, matryca RTO/RPO, procedury awaryjne/workarounds, plan komunikacji, harmonogram testów/ćwiczeń, rejestr lessons learned.

## Powiązania (meta)
- Business Impact Analysis (BIA)
- Disaster Recovery Plan (DRP) i Disaster Recovery Testing
- Incident Response Plan (cyber/operacyjny)
- Runbooki usług (app/DB/network/identity)
- Standardy: ISO 22301, ISO 27001 A.17, SOC2 CC7

## Zależności dokumentu
- DRP / DR Testing (RTO/RPO systemów i wyniki testów)
- BIA (priorytety procesów i skutki przerw)
- Runbooki usług krytycznych i plan komunikacji kryzysowej
- SLA z dostawcami krytycznymi
...
1. Kontekst i krytyczne procesy (BIA, właściciele, priorytety).
2. Scenariusze zakłóceń i założenia (personel, lokalizacje, dostawcy, IT).
3. RTO/RPO procesów i minimalne poziomy usług.
4. Workarounds / procedury awaryjne (lokacje zapasowe, tryby manualne).
5. Organizacja kryzysowa i komunikacja (role, kontakty, kanały, szablony).
6. Integracja z DRP/IR (punkty wyzwalające DR; współpraca z cyber/operacyjny IR).
7. Testy i ćwiczenia BCP (częstotliwość, zakres, metryki, rejestr akcji).
8. Utrzymanie planu (przeglądy, aktualizacja kontaktów i danych BIA, onboarding nowych dostawców/procesów).
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Disaster Recovery Plan (DRP)', 'type': 'REFERENCES'}
- Zależność: {'depends_on': 'Disaster Recovery Plan', 'type': 'REFERENCES'}
- Zależność: {'depends_on': 'Disaster Recovery Testing', 'type': 'REFERENCES'}
