---
title: Performance Incident Postmortem
...
## Cel dokumentu
Przeanalizować incydent wydajności (latencja, throughput, resource exhaustion), zidentyfikować wąskie gardła i wdrożyć trwałe usprawnienia.

## Zakres i granice
Obejmuje: timeline, wpływ na SLO/SLA, komponenty dotknięte, root cause (kod/infra/config/data), testy/perf, CAPA. Nie obejmuje: pełnej strategii performance (ta jest w Performance Test Plan/NFR).

## Wejścia i wyjścia
- Wejścia: metryki i alerty (p95/p99, error rate), trace’y, profile, zmiany deployu, ruch/traffic patterns, konfiguracje zasobów.
- Wyjścia: postmortem performance, lista działań (optimizacja kodu/zapytań, cache, limits, autoscaling), aktualizacje testów perf, monitoringu i SLO.
...
- Streszczenie i wpływ na SLO/SLA
- Timeline (zmiany, ruch, alerty)
- Root cause + contributing (kod, baza, infra, konfiguracja, dane)
- Diagnostyka: co wykryto / co zawiodło (monitoring, tests, canary)
- CAPA: optymalizacje, limity, skalowanie, cache, retry/backoff
- Testy regresyjne i walidacja po poprawkach
