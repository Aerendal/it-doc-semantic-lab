---
title: Compliance Incident Postmortem
...
# Compliance Incident Postmortem
...
## Cel dokumentu
Przeanalizować incydent naruszenia wymogów regulacyjnych/standardów (np. GDPR, SOC2, ISO), ustalić przyczyny i zaplanować działania korygujące, by przywrócić zgodność i ograniczyć ryzyko kar.

## Zakres i granice
Obejmuje: timeline, podatności/procesy, wpływ na dane/PII, obowiązki raportowe (regulator, klienci), CAPA, aktualizacje kontroli i dokumentacji. Nie obejmuje: pełnej polityki compliance (osobny dokument).

## Wejścia i wyjścia
- Wejścia: raport z Incident Response, logi bezpieczeństwa, rejestr PII/danych, wymagania regulacyjne, umowy/SLA, audyt wewn./zewn.
- Wyjścia: postmortem compliance, lista CAPA (kontrole, procesy, szkolenia, dokumentacja), plan raportowania do regulatorów/klientów, aktualizacje rejestrów danych.
...
- Compliance Incident Response (sequential)
- Compliance Incident Response (sequential)
...
- Streszczenie i wpływ (dane/PII, systemy, klienci, jurysdykcje)
- Timeline faktów i decyzji
- Wymogi naruszone i analiza przyczyn (techniczne, procesowe, organizacyjne)
- Obowiązki raportowe i status (regulatorzy, klienci, SLA)
- CAPA: kontrole, procesy, szkolenia, dokumentacja, audyt
- Aktualizacje dokumentów compliance i rejestrów danych
- Follow-up i status działań
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Compliance Incident Response', 'type': 'sequential'}
- Zależność: {'depends_on': 'Compliance Incident Response', 'type': 'sequential'}
