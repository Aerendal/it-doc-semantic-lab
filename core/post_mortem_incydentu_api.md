---
title: Post-mortem incydentu API
...
Przeanalizować incydent dotyczący API (błędy, degradacje, przerwy) i wdrożyć trwałe poprawki.
...
1. Podsumowanie: czas, wpływ (endpoints/klienci), SLA, error codes/latency.
2. Timeline: zdarzenia, alerty, deploye, decyzje, komunikacja do konsumentów.
3. Root cause: kod/konfiguracja/integracje, rate limit, upstream deps; contributory factors.
4. CAPA: fixy, testy kontraktowe/perf, polityki (rate limit/cache), właściciele, terminy.
5. Usprawnienia: monitoring/logging/tracing dla API, fallbacki, wersjonowanie, chaos tests.
6. Follow-up: retest, aktualizacja spec/portal, komunikacja, status SLA/error budget.
...
- [ ] Timeline fact-based; root cause API zidentyfikowane.
- [ ] CAPA obejmują kod/konfig/testy/perf/rate limit; właściciele i terminy nadane.
- [ ] Monitoring/tracing i fallbacki wzmocnione; spec/portal zaktualizowane.
- [ ] Retest i komunikacja do konsumentów przeprowadzone.
