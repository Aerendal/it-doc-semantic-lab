...
## Cel dokumentu
Określić proces tworzenia modeli 3D (high/low poly, retopo, bake, UV, mapy PBR, kolizje, LOD, eksport) tak, aby modele były spójne artystycznie, zgodne technicznie z pipeline’m i spełniały budżety wydajności platform docelowych.

## Zakres i granice
- Obejmuje: workflow high→low/retopo, bake (normal/AO/curvature), UV/texel density, mapy PBR, LOD, kolizje, naming, export/import settings, QA checklist, re-use/naming conventions.
- Poza zakresem: rig/animacja (oddzielne dokumenty) oraz design środowiska (oddzielne).

## Wejścia i wyjścia
- Wejścia: concept/refy, styl/Art Bible, skala/budżety, 3D Asset Design Spec, shader/material guide, perf targety, wymagania kolizji/gameplay.
- Wyjścia: pliki źródłowe (hip/blend/max), export (fbx/gltf), mapy (albedo/normal/rough/metal/AO/emissive/opacity), LOD-y, colliders, QA report.
...
- Plan i referencje: mood/scale, ref board.
- High/Low i retopo: poly budget, topo zasady, silhouette, bevel.
- Bake: mapy (normal/AO/curvature/thickness), cage, antialiasing, formaty/bit depth.
- UV i texel density: UDIM jeśli, padding, overlap policy, kanał UV2 (lightmap gdy potrzebne).
- Mapy i materiały: PBR metal/rough, wymagane mapy, kolor space, formaty.
- LOD: liczba, target tri per LOD, naming, kąty/progi, HLOD jeśli.
- Kolizje: typ (simple/convex/mesh), pivot/origin, attach points.
- Eksport/import: format, axis/scale, naming, foldery, smoothing groups, tangent space.
- QA checklist: wireframe, shading, UV, mapy, LOD pop, kolizje, texel density, draw-call.

## Wymagane rozwinięcia
- Styl → Art Bible / 3D Asset Design Spec.
- Materiały/shadery → Material/Shader Guide.
- LOD/perf → Graphics Architecture / Performance Budget.
- Kolizje → Gameplay/Physics spec.

## Wymagane streszczenia
- Streszczenie budżetu (tri, mapy, LOD), formatów eksportu i kolizji.
...
- DoR:
  - [ ] Concept/refy i styl dostępne; budżety/skala znane.
  - [ ] Wytyczne materiałów/shaderów oraz spec kolizji/LOD dostępne.
- DoD:
  - [ ] Model spełnia budżet tri, UV/TD, mapy PBR, LOD i kolizje; eksport zgodny z pipeline.
  - [ ] QA checklist (wireframe/UV/mapy/LOD/collision) przejrzana; sekcje N/A uzasadnione; metadane aktualne.
