---
title: Cutover linii produkcyjnych
...
Zaplanować i przeprowadzić bezpieczny cutover linii produkcyjnych (OT/IT): minimalizacja przestojów, ryzyka i zapewnienie rollbacku.
...
1. Zakres i okno cutover (linie/urządzenia/procesy).
2. Uwarunkowania produkcyjne (cykl, zamówienia, bezpieczeństwo pracy).
3. Plan techniczny (kroki, sekwencja, zależności, lockout/tagout jeśli dotyczy).
4. Testy przed/po (dry-run, SAT, verifikacja sygnałów/PLC/SCADA).
5. Plan rollback i kryteria abort.
6. Role i komunikacja (OT/IT/produkcja, kanały).
7. Monitoring i walidacja po cutover (OEE, alarmy, jakość).
...
- [ ] Okno i zakres uzgodnione z produkcją.
- [ ] Plan kroków + dry-run/SAT wykonane.
- [ ] Rollback i abort criteria opisane.
- [ ] Role/komunikacja OT/IT ustalone.
- [ ] Walidacja po cutover (OEE, alarmy) przeprowadzona.
