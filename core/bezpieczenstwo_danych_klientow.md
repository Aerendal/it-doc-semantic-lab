...
Zapewnić ochronę danych klientów (PII/PHI/finansowe) w całym cyklu życia: zbieranie, przetwarzanie, przechowywanie, udostępnianie i usuwanie.


## Zakres i granice

- Obejmuje: klasyfikację danych, minimalizację, szyfrowanie w tranzycie/at rest, dostęp (least privilege), logowanie i anonimizację, retencję/usuwanie, udostępnianie partnerom, incydenty i zgłoszenia naruszeń.
- Nie obejmuje: pełnych procedur prawnych/RODO (osobny dokument) ani bezpieczeństwa płatności PCI (osobny).
...
1. Klasyfikacja i minimalizacja danych klienta.
2. Zbieranie i zgody (legal basis).
3. Przechowywanie i szyfrowanie (tranzyt/at rest, klucze).
4. Dostęp i logowanie (RBAC/ABAC, audyt).
5. Udostępnianie partnerom (DPA, minimal scope, transfery).
6. Retencja/usuwanie i prawa podmiotów danych.
7. Incydenty i zgłoszenia (proces, SLA).
...
- [ ] Dane sklasyfikowane, minimalizacja opisana.
- [ ] Szyfrowanie tranzyt/at rest i zarządzanie kluczami.
- [ ] Dostępy rolami i audyt logów.
- [ ] Retencja/usuwanie zgodne z polityką.
- [ ] Proces incydentów i zgłoszeń opisany.
