---
title: Model Performance Postmortem
...
## Cel dokumentu
Przeanalizować degradację lub incydent wydajności modelu ML (jakość predykcji, drift, opóźnienia inferencji), znaleźć przyczyny i ustalić działania naprawcze.

## Zakres i granice
Obejmuje: timeline, metryki jakości (AUC/F1/latencja), drift danych/feature’ów, zmiany modelu/feature store, skale obciążenia, CAPA. Nie obejmuje: całego cyklu MLOps (jest w MLOps docs).

## Wejścia i wyjścia
- Wejścia: monitoring modelu (quality/latency), feature store logi, dane walidacyjne, zmiany modelu/wersji, ruch/traffic patterns.
- Wyjścia: postmortem modelowy, działania (retrain/recalibrate, feature fixes, thresholds, infra), aktualizacje monitoringu/alertów, plan eksperymentów/regresji.
...
- Model Degradation Response (sequential)
- Model Degradation Response (sequential)
...
- Streszczenie i wpływ (użytkownicy, KPI biznesowe)
- Timeline (deploy wersji, zmiany feature store, ruch)
- Root cause + contributing (drift, data quality, concept drift, infra latency)
- Metryki jakości i obserwowalność modelu (przed/po, alerty)
- CAPA: dane/feature, model (retrain/recalib), thresholds, cache/batching, autoscaling
- Walidacja i testy regresyjne, plan rollout
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Model Degradation Response', 'type': 'sequential'}
- Zależność: {'depends_on': 'Model Degradation Response', 'type': 'sequential'}
