...
Opisać architekturę pipeline’u danych/przetwarzania: komponenty, przepływy, niezawodność i bezpieczeństwo.
...
1. Widok wysokopoziomowy: źródła → ingest → przetwarzanie → storage → konsumenci.
2. Komponenty: kolejkowanie/bus, ETL/ELT, transform, quality checks, storage (lake/warehouse), serving (API/BI/ML).
3. Niezawodność: retry/idempotencja, DLQ, check-pointing, SLA, backpressure.
4. Bezpieczeństwo: auth/roles, szyfrowanie, maskowanie, audyt, sekret management.
5. Observability: logi/metryki/traces, lineage, DQ monitoring, alerty.
6. Operacje: IaC/deploy, wersjonowanie schematów, change management, runbooki.
...
- [ ] Przepływy i komponenty pipeline opisane (źródła→konsumenci).
- [ ] Mechanizmy niezawodności (retry/DLQ/checkpoints) wdrożone.
- [ ] Bezpieczeństwo (auth/szyfrowanie/sekrety) uwzględnione.
- [ ] Observability/lineage i change management skonfigurowane.
