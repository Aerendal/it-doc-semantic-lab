...
## Cel dokumentu
Zaplanować strukturę danych i schemat bazy tak, aby wspierała wymagania funkcjonalne i niefunkcjonalne (wydajność, bezpieczeństwo, zgodność). Uzgadnia nazwy tabel/kolumn, klucze, indeksy, relacje i zasady retencji.

## Zakres i granice
Obejmuje: model konceptualny/logiczny/fizyczny, normalizację/denormalizację, klucze, indeksy, widoki, partycjonowanie, strategię wersjonowania schematu, polityki danych wrażliwych (PII), backup/retencję. Nie obejmuje: szczegółów implementacji warstwy aplikacyjnej.

## Wejścia i wyjścia
- Wejścia: wymagania danych z FRS, NFR (wydajność/skalowalność), profile obciążenia, wymagania bezpieczeństwa/compliance, decyzje architektoniczne.
- Wyjścia: zatwierdzony schemat (DDL), plan migracji, indeksy i klucze, plan shardingu/partycjonowania, wymagania do testów wydajności i integralności.
...
- Konsumuje: Database Requirements Spec, Performance/Capacity requirements, Security/Privacy requirements.
- Dostarcza: DDL/migracje do repo, dane wejściowe do Performance Test Plan, Backup/DR Runbook.

## Powiązania sekcja↔sekcja
- Model logiczny ↔ mapowanie do fizycznego (typy, indeksy).
- Partycjonowanie/sharding ↔ strategia migracji i HA/DR.
- Dane wrażliwe ↔ polityki maskowania/szyfrowania i audyt.
...
- Cel, zakres, założenia obciążenia
- Model konceptualny i słownik pojęć
- Model logiczny (tabele/relacje, klucze)
- Model fizyczny (typy, indeksy, partycje, storage)
- Zasady integralności i walidacje
- Dane wrażliwe: klasyfikacja, szyfrowanie, maskowanie, audyt
- Retencja, archiwizacja, backup/restore, DR
- Strategia migracji i zgodność wsteczna (rolling deploy)
- Ryzyka i metryki (np. koszty zapytań, czas odczytu/zapisu)
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Database Requirements Specification', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Performance Requirements', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Database Requirements Specification', 'type': 'INFORMED_BY'}
