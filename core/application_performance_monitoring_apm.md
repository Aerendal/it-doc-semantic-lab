...
Opisać, jak monitorujemy wydajność aplikacji (backend/web/mobile): metryki, trace’y, logi, alerty, by szybko wykrywać i diagnozować problemy.


## Zakres i granice

- Obejmuje: metryki (latencja, throughput, error rate), distributed tracing, logi strukturalne, sampling, alerty, korelację z deploymentami.
- Nie obejmuje: pełnych planów optymalizacji (osobny dokument) ani szczegółowej konfiguracji bezpieczeństwa logów (jest w Security/PII Logging).
...
- Wejścia: SLO/SLI, architektura usług, ruch produkcyjny, narzędzia APM/observability.
- Wyjścia: zestaw metryk i trace’ów, dashboardy, alerty, procedury triage i runbooki.
...
1. Zakres usług i środowisk (prod/stage).
2. Metryki: latencja (p50/p95/p99), throughput, error rate.
3. Tracing: sampling, baggage/metadata, korelacja z logami.
4. Logowanie: format, kontekst, redakcja PII, retencja.
5. Dashboardy i alerty (progi, kanały, właściciele).
6. Procedury triage (co sprawdzić, kiedy eskalować).
...
- [ ] Metryki/SLO zdefiniowane (p95/p99, error rate).
- [ ] Tracing i logi skorelowane (trace_id).
- [ ] Alerty z progami i właścicielem.
- [ ] Procedura triage opisana.
- [ ] Retencja i redakcja PII uwzględnione.


## Definicje robocze

- **APM** — narzędzia do śledzenia wydajności i stabilności aplikacji.
- **Trace** — ścieżka żądania przez usługi; pozwala znaleźć wąskie gardła.
...
- „Metryki i trace’y” **constrains** „Alerty”.
- „Deployment korelacja” **feeds** „Diagnoza regresji”.
