...
## Cel dokumentu
Krótka definicja: QA Project Schedule opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
- Test Strategy Document (INFORMED_BY)
- Test Case Specification (INFORMED_BY)
- Test Strategy Document (INFORMED_BY)
- Test Case Specification (INFORMED_BY)
...
- Kamienie milowe QA: przygotowanie danych/środowisk, smoke, regresja, UAT, performance window, security, sign-off, go/no-go.
- Oś czasu i zależności: Gantt lub tabela z datami, zależnościami (dev freeze, release candidate, okna maintenance).
- Rezerwacja zasobów: zespoły, sprzęt, środowiska, runnerzy CI; kto, na kiedy, w jakim wymiarze.
- Bufory ryzyka: rezerwy czasowe na flaki, naprawy krytycznych defektów, opóźnienia środowiskowe.
- Integracje i zewnętrzne okna: dostępność vendorów, 3rd party, sandboxów, certyfikacji.
- Kryteria wejścia/wyjścia per etap: co musi być spełnione przed przejściem do kolejnej fazy (DoR/DoD).
- Komunikacja: rytuały (triage, daily QA, release readiness), odbiorcy raportów, ścieżki eskalacji.

## Wymagane rozwinięcia
- Kamienie milowe rozwijają daty z Release Plan i Test Strategy.
- Rezerwacja zasobów rozwija: Resource Allocation for QA, CI/CD Capacity Plan.
- Bufory ryzyka rozwijają: Risk Register, Environment Readiness, Known Flaky Tests.

## Wymagane streszczenia
- Streszczenie harmonogramu dev/release (z Release Plan) ograniczone do punktów wpływających na QA.
- Streszczenie dostępności środowisk i 3rd party (z Environment Runbook / Vendor Calendar).
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Test Strategy Document', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Test Case Specification', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Test Strategy Document', 'type': 'INFORMED_BY'}
...
- DoR:
  - [ ] Kamienie milowe i daty uzgodnione z Release Managerem/Prod/Dev.
  - [ ] Zależności (freeze, okna maintenance, vendor/sandbox) zmapowane.
  - [ ] Dostępność środowisk i zasobów (QA, Dev, Sec, Ops) potwierdzona.
- DoD:
  - [ ] Harmonogram zawiera bufory na ryzyka/flaki; kryteria wejścia/wyjścia per etap są jasne.
  - [ ] Rezerwacje środowisk/runnerów zapisane; kolizje rozwiązane.
  - [ ] Plan komunikacji i eskalacji dołączony; dokument wersjonowany i udostępniony.
