...
Zaplanować i opisać pipeline renderingu (kolejność etapów, dane, zasoby) tak, aby osiągnąć cele jakości i wydajności na docelowych platformach.


## Zakres i granice

- Obejmuje: etapy culling → geometry → lighting/shading → post‑FX → UI, layout buforów, kolejność passów, synchronizację CPU/GPU, multi-threading, kompresję/formaty zasobów, debug/profiling.
- Poza zakresem: asset pipeline i style artystyczne (osobne dokumenty).
...
- Model pipeline i kolejność etapów
- Dane i zasoby (bufory, tekstury, formaty)
- Synchronizacja CPU/GPU i wątki
- Platformowe warianty/fallbacki
- Debug/profiling i telemetria
- Ryzyka i decyzje


## Szybkie powiązania
- rendering-pipeline-reference
- programmatic-pipeline-design
- pipeline-design-patterns
- inference-pipeline-design
- design-pipeline-u-programmatic
...
- Cele FPS/frametime i profile jakości
- Target API (DX12/Vulkan/Metal/OpenGL) i platformy
- Budżety GPU/CPU/VRAM, ograniczenia driverów
- Wymagania narzędzi (capture, hot-reload shaderów)


## Wyjścia

- Diagram pipeline i opis etapów
- Zasady kolejności i priorytetów passów
- Zasady alokacji/formatów zasobów (RT/DS/GBuffers)
- Lista punktów pomiaru i narzędzi debug/profiling
...
- [ ] API/platformy wybrane
- [ ] Budżety GPU/CPU/VRAM zebrane
- [ ] Sceny referencyjne do pomiaru gotowe


## Kryteria DoD

- [ ] Pipeline i kolejność opisane
- [ ] Formaty/zasoby i synchronizacja zdefiniowane
- [ ] Warianty/fallbacki spisane
...
- Lista passów i zasobów RT/DS/GBuffers
- Capture z profilera (CPU/GPU)
...
- Czas kompilacji/ładowania shaderów
