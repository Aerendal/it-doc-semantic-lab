...
## Cel dokumentu
Określić, jak i kogo powiadamiać podczas incydentów, aby informacje były szybkie, spójne i zgodne z politykami bezpieczeństwa/PR.

## Zakres i granice
Obejmuje: listy odbiorców (wew./zew.), kanały i częstotliwość powiadomień, szablony wiadomości, zasady informacji wrażliwych, wymogi audytowe. Nie obejmuje: szczegółowych kroków technicznych reakcji (runbooki).

## Wejścia i wyjścia
- Wejścia: klasyfikacja/severity incydentu, Incident Response Plan, Incident Communication Plan, polityki PR/legal, status techniczny, wymagania regulatorów/umów SLA.
- Wyjścia: harmonogram powiadomień, szablony komunikatów, lista odbiorców i właścicieli wysyłek, dziennik wysłanych powiadomień z potwierdzeniami.

## Powiązania (meta)
- Incident Response Playbook / Plan\n- Crisis Communication Template / Regulator comms\n- DRP/BCP (jeśli dotyczy outage)\n- Polityki PR/Legal/Compliance (wymagania treści, okna zgłoszeń)\n- SLA z klientami (wymagane powiadomienia/ETA)

## Zależności dokumentu
- Klasyfikacja incydentu (SEV) i status techniczny\n- Lista odbiorców (wew./zew./regulatorzy) i kanały\n- Szablony komunikatów i zatwierdzenia PR/Legal\n- Rejestr powiadomień i potwierdzeń odbioru
...
1. Zakres i zasady (jakie incydenty wymagają powiadomień; kto uruchamia komunikację).\n2. Listy odbiorców i kanały (wew./zew./regulatorzy/klienci; e-mail, status page, SMS, ticketing).\n3. Częstotliwość i triggery aktualizacji (severity → cadence; przed/po RTO; zmiana wpływu).\n4. Szablony komunikatów (initial, update, resolved, RCA zapowiedź) z polami obowiązkowymi i zakazanymi.\n5. Zasady informacji wrażliwych i aprobaty (PR/legal/security; kto zatwierdza, kiedy można publikować dane techniczne).\n6. Dziennik komunikacji i audyt (timestamp, odbiorcy, kanał, potwierdzenie odbioru, link do ticketu/incydentu).

## Wymagane rozwinięcia
- Mapowanie severity → cadence komunikacji i kanałów.\n- Szablony komunikatów z placeholderami: wpływ, RTO/ETA, działania, kontakt.\n- Zasady informacji wrażliwych (co można ujawnić, komu, kiedy; PII/PCI/HIPAA/sekret firmowy).\n- Wymagania regulatorów/klientów (okna zgłoszeń, format, język).\n\n## Wymagane streszczenia\n- Streszczenie ścieżki akceptacji komunikatów (PR/Legal/Security) i kanałów.\n- Streszczenie „minimum viable update” dla SEV1/SEV2 (co musi być w każdej aktualizacji).
