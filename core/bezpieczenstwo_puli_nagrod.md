---
title: Bezpieczeństwo puli nagród
...
Zabezpieczyć środki/pule nagród (np. w turniejach, programach lojalnościowych) przed kradzieżą, nadużyciem i błędami operacyjnymi.


## Zakres i granice

- Obejmuje: model uprawnień do wypłat, zasady akumulacji i dystrybucji, segregację obowiązków (SoD), audyt i logging, kontrole anty-fraud, zgodność regulacyjna (płatności/AML jeśli dotyczy).
- Nie obejmuje: szczegółów procesora płatności (osobny dokument) ani marketingu programu nagród.
...
1. Źródła i przeznaczenie puli, model własności.
2. Uprawnienia i proces wypłat (role, SoD, zatwierdzenia).
3. Integracja z płatnościami/portfelem (limity, 2FA/MFA).
4. Monitoring i audyt (logi, alerty, reconciliation).
5. Anti-fraud i blokady (limity dzienne, velocity, KYC/AML gdy dotyczy).
...
- [ ] Role/SoD i proces wypłat opisane.
- [ ] Limity, MFA/2FA, zatwierdzenia wielostopniowe.
- [ ] Logi/audyt i reconciliation działają.
- [ ] Reguły anty-fraud/KYC/AML (jeśli wymagane) wdrożone.
