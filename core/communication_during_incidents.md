...
## Cel dokumentu
Krótka definicja: Communication During Incidents opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
- Zakres i klasy incydentów: sev/P poziomy, co komunikujemy publicznie vs wewnętrznie.
- Role i RACI: Incident Commander, Comms Lead, Technical Lead, Legal/PR, Customer Support.
- Kanały komunikacji: war room (chat/call), status page, e‑mail, ticketing, social/PR; zasady użycia i fallback.
- Harmonogram i szablony: częstotliwość update’ów per sev, gotowe szablony (wewnętrzne, status page, klient, PR).
- Zasady treści: fakty vs hipotezy, język, NDA/regulatorzy, dane wrażliwe, co wolno/nie wolno ujawniać.
- Eskalacja i zatwierdzenia: kto akceptuje komunikaty, ścieżki backup, sign-off dla zewnętrznych komunikatów.
- Logowanie i zgodność: kto zapisuje timeline, gdzie trzymamy kopie komunikatów, odniesienie do postmortem.

## Wymagane rozwinięcia
- Role/eskalacja rozwija: Incident Response Playbook, Design procedur eskalacji.
- Szablony/harmonogram rozwijają: Communication Plan for Incidents, Status Page Playbook.
- Zasady treści rozwijają: Legal/PR guidelines, Privacy/Compliance (RODO/HIPAA, jeśli dotyczy).

## Wymagane streszczenia
- Streszczenie częstotliwości i kanałów per sev (tabela) oraz kto akceptuje które komunikaty.
...
- DoR:
  - [ ] Sev/klasy incydentów i wymagane kanały per sev uzgodnione.
  - [ ] Role (IC, Comms Lead, Tech Lead, Legal/PR, Support) przypisane wraz z zastępstwami.
  - [ ] Szablony komunikatów dostępne (wewnętrzne/status page/klient/PR).
- DoD:
  - [ ] Tabela częstotliwości/kanałów per sev gotowa; zasady treści (co/nigdy nie mówimy) opisane.
  - [ ] Ścieżki eskalacji/akceptacji opisane; logowanie timeline i archiwizacja komunikatów wskazane.
  - [ ] Dokument wersjonowany, udostępniony zespołom, punkt przeglądu okresowego zapisany.
