...
Zebrać typowe problemy w AdTech (delivery/measurement/privacy) i podać szybkie rozwiązania.
...
- [ ] Definicje metryk uzgodnione (timezone, viewability).
- [ ] Fraud/brand safety włączone.
- [ ] CMP i zgody wdrożone, cookies/ID zgodne z polityką.
- [ ] Latency tagów monitorowana, optymalizacje wdrożone.
- [ ] Attribution model uzgodniony.
...
- **Discrepancy impressions/clicks**: różne definicje, ad blockers, latency → ujednolić definicje, filtrować bots, sync timezone.
- **Viewability niska**: placement, lazy load → popraw layout, lazy load, zgodność z MRC.
- **Brand safety/fraud**: brak filtrów → włącz fraud detection, blocklist/allowlist, ads.txt/app-ads.txt.
- **Latency tagów**: zbyt wiele requestów → sekwencjonowanie, async tags, CDN.
- **Privacy/consent**: brak CMP → wdrożyć CMP (TCF), honorować zgody/opt-out, minimalizacja danych.
- **Attribution**: niespójne okna/kanały → uzgodnić window/last-touch vs multi-touch, server-side tracking gdy możliwe.
