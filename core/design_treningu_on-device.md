---
title: Design treningu on-device
...
Zaprojektować trening modeli na urządzeniu (federated/local): dane, prywatność, komunikacja i aktualizacje.
...
1. Use case i model, dane lokalne (co zostaje na device).
2. Federated loop: init, local train, upload, aggregation; lub full local.
3. Prywatność/bezpieczeństwo: DP, secure aggregation, mTLS, storage szyfrowany.
4. Komunikacja: harmonogram rund, sampling klientów, tolerancja awarii.
5. Zasoby urządzenia: CPU/battery/memory constraints, quantization.
6. Aktualizacje/rollback modeli, wersje i audyt.
7. Observability: participation, loss/metrics, drift, budżet DP.
...
- [ ] Federated/local flow opisany.
- [ ] DP/secure agg i mTLS wdrożone.
- [ ] Budżet battery/CPU i quantization uwzględnione.
- [ ] Rollout/rollback modeli i monitoring rund.
