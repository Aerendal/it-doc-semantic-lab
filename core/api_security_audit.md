...
Przeprowadzić audyt bezpieczeństwa API (kontrole procesów i konfiguracji) zgodnie z politykami i standardami.


## Zakres i granice

- Obejmuje: przegląd polityk authn/authz, rate limiting, logowanie/audyt, zarządzanie kluczami/secrets, zgodność z OWASP API Top 10, PCI/PII jeśli dotyczy.
- Poza zakresem: testy penetracyjne (oddzielne dokumenty).
...
- Kontekst i zakres
- Kontrole: authn/authz, rate limiting, logging, keys/secrets
- Zgodność z OWASP/API i regulacjami
- Wyniki i ryzyka
- Rekomendacje i plan
- Ryzyka i follow-up
...
- Polityki bezpieczeństwa i konfiguracje gateway
- Spec API i modele uprawnień
- Logi/audyt, wyniki wcześniejszych testów
- Zakres regulacyjny (PCI/PII/GDPR)


## Wyjścia

- Raport audytu z ustaleniami
- Lista niezgodności i plan naprawczy
- Mapa dowodów i artefaktów
- Powiązania do runbooków/testów
...
- [ ] Zakres audytu i standardy zdefiniowane
- [ ] Dostęp do konfiguracji/logów zapewniony
- [ ] Modele uprawnień zebrane
- [ ] Ownerzy bezpieczeństwa ustaleni


## Kryteria DoD

- [ ] Raport i niezgodności spisane
- [ ] Plan naprawczy z ownerami/SLA
...
- Liczba niezgodności High/Med
- Czas do zamknięcia zaleceń
- Pokrycie kontrolami
