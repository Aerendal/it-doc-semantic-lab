...
Typowe problemy z POS (punkt sprzedaży) i praktyczne rozwiązania.
...
- [ ] Failover sieci/VPN i retry z limitami.
- [ ] Aktualizacje/whitelist, monitoring zdrowia.
- [ ] Idempotent transaction ID; reconciliation dzienny.
- [ ] Tokenizacja/E2E, segmentacja POS.
- [ ] Drukarki/urządzenia monitorowane z fallbackiem.
...
- **Brak łączności z PSP**: sieć/VPN down → failover link, retry/backoff, offline queue z limitami, alerty.
- **Zawieszanie terminala**: firmware stare → patching, watchdog, whitelisting, monitoring health.
- **Duplikaty transakcji**: retry bez idempotencji → unique transaction ID, server-side auth/capture.
- **Rozliczenia niezgodne**: brak reconciliation → dzienne zestawienia, automatyczny reconciliation z PSP.
- **PCI scope**: dane karty w POS → tokenizacja, E2E encryption, segmentacja sieci.
- **Drukarki/kolejki**: offline → health checks, buffer, fallback printer.
