...
Zapewnić bezpieczeństwo procesów płatniczych (online/offline), zgodność z PCI i ochronę użytkowników przed fraudem.


## Zakres i granice

- Obejmuje: integracje z PSP/acquirer, tokenizację, 3DS/SCA, PCI scope i segmentację, przechowywanie danych płatniczych (lub unikanie), logowanie/monitoring transakcji, anti-fraud, chargeback handling.
- Nie obejmuje: polityk cenowych ani marketingu.
...
1. Architektura płatności i PCI scope (co wchodzi/wychodzi).
2. Dane płatnicze i tokenizacja (gdzie trzymane, jak zabezpieczone).
3. Autentykacja/Autoryzacja (3DS/SCA), limity, retry.
4. Monitoring i anti-fraud (velocity, device fingerprint, rules/ML).
5. Incident/chargeback handling i raporty.
...
- [ ] PCI scope określony, dane kart nieprzechowywane bez potrzeby.
- [ ] Tokenizacja i 3DS/SCA wdrożone.
- [ ] Anti-fraud i monitoring transakcji działa.
- [ ] Proces chargeback/incident opisany.
