...
Opisać architekturę Change Data Capture: źródła, log capture, przetwarzanie, dostarczanie i konsumpcja zmian z gwarancjami spójności.


## Zakres i granice

- Obejmuje: mechanizmy capture (binlog/redo/log-based/trigger), kolejkę/stream (Kafka/Kinesis), format zdarzeń (envelope, schema), ordering/dup/latency, idempotencję, monitoring, schemat ewolucji.
- Nie obejmuje: pełnej architektury danych downstream (osobne dokumenty) ani szczegółów pojedynczych źródeł.
...
1. Źródła i mechanizmy capture (DB, log, trigger).
2. Transport/stream (broker, ordering, partitioning, retries).
3. Schemat i format zdarzeń (envelope, schema registry, evolucja).
4. Dostarczanie/consumption (sink, exactly-once/idempotencja, upsert/merge).
5. Monitoring i SLA (lag, error rate, data quality).
6. Bezpieczeństwo (auth, szyfrowanie, PII masking).
...
- [ ] Źródła i mechanizm capture opisane.
- [ ] Broker/ordering/retry skonfigurowane.
- [ ] Schemat + evolucja z registry.
- [ ] Idempotencja/upsert w sink.
- [ ] Monitoring lag/error i bezpieczeństwo.
