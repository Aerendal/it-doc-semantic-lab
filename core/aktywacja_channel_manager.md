---
title: Aktywacja channel manager
...
Opis kroków technicznych i operacyjnych potrzebnych do uruchomienia channel managera (np. w hotelarstwie/booking), aby zsynchronizować dostępność, ceny i rezerwacje między systemami.


## Zakres i granice

- Obejmuje: konfigurację połączeń z OTA/PMS/CRS, mapowanie pokoi/stawek, synchronizację dostępności i rezerwacji, testy end-to-end, procedurę cutover.
- Nie obejmuje: strategii revenue management (osobny dokument), ogólnej polityki cenowej.
...
- Wejścia: dane obiektu/pokoi/stawek, klucze API do OTA/PMS, polityki anulacji, harmonogram aktualizacji cen.
- Wyjścia: aktywny channel manager, potwierdzone mapowanie kanałów, checklisty testów, plan fallback (manual override), instrukcja operacyjna.
...
- Konfiguracja PMS/CRS.
- Polityki cen i dostępności (Revenue/Pricing policy).
- Integracja płatności/PCI (jeśli dotyczy).
...
1. Wymagania wstępne (umowy API, dane obiektu, konta OTA).
2. Konfiguracja połączeń (PMS ↔ CM ↔ OTA; klucze, webhooki).
3. Mapowanie pokoi/stawek i restrykcji.
4. Synchronizacja i harmonogram (availability, rates, restrictions).
5. Testy end-to-end (rezerwacja, modyfikacja, anulacja, overbooking).
6. Cutover i fallback (kiedy przełączamy, jak wrócić do manuala).
7. Operacje bieżące (monitoring błędów, rotacja kluczy, support).
...
- [ ] Mapowanie pokoi/stawek zweryfikowane w każdym kanale.
- [ ] Testy E2E: create/modify/cancel, overbooking prevention.
- [ ] Harmonogram sync i limity API ustawione.
- [ ] Fallback/rollback opisany i przetestowany.
- [ ] Monitoring błędów i alerty skonfigurowane.
...
- „Mapowanie pokoi/stawek” **defines_structure** „Testy synchronizacji”.
- „Procedura cutover” **guides_expansion** „Plan rollback/fallback”.
