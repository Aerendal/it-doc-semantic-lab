...
Playbook reagowania na incydent naruszenia danych: wykrycie, triage, dochodzenie, ograniczenie skutków, notyfikacje prawne/klienckie, remediacja i postmortem. Ma ograniczyć skalę szkody, zapewnić zgodność regulacyjną i spójność komunikacji.


## Zakres i granice

- Obejmuje: typy naruszeń (PII/PHI/PCI/IP), kanały wykrycia, klasyfikację wstępną, zabezpieczenie dowodów, analizę przyczyn, containment/eradication, notyfikacje regulatorów/klientów/partnerów, komunikację (PR/CS), remediację i lekcje, integrację z IR/BCP.
- Poza zakresem: pełny forensics laboratory (może być referencja), stała ochrona danych (baseline w innych dokumentach).
...
- Klasyfikacja danych/jurysdykcji → Notyfikacje/regulator → Treść i SLA.
- Containment → Remediacja → Postmortem → Aktualizacja kontroli.
- Dowody/forensics → Raport regulatora/klienta → Audyt i lekcje.


## Struktura sekcji

1) Scenariusze i kanały wykrycia (SIEM, zgłoszenie klienta, bug bounty)  
2) Klasyfikacja danych i jurysdykcji (PII/PHI/PCI, kraje)  
3) Triage i decyzja breach/not (kryteria, próg, role)  
4) Containment i zabezpieczenie dowodów (chain-of-custody)  
5) Analiza przyczyn (RCA) i zakres danych/osób/systemów  
6) Notyfikacje i komunikacja (regulator, klienci, partnerzy, PR)  
7) Remediacja i monitorowanie po incydencie  
8) Postmortem i lekcje; aktualizacja kontroli/procedur  
