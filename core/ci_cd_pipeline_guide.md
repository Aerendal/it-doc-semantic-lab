...
Opisać standardy budowy i utrzymania pipeline’ów CI/CD: budowa, testy, bezpieczeństwo, deploy, obserwowalność i rollback.


## Zakres i granice

- Obejmuje: etapy CI (lint/build/test/scan), artefakty, wersjonowanie, gating, environment promotion, secrets, compliance, monitoring pipeline, retry/fail-fast, manual approvals.
- Nie obejmuje: szczegółowej konfiguracji konkretnych narzędzi (GitLab/GH Actions/Jenkins) — można linkować przykłady.
...
- [ ] Etapy CI/CD i bramki jakości ustawione.
- [ ] SAST/DAST/dependency scan/secrets włączone.
- [ ] Artefakty wersjonowane/podpisane, SBOM generowany.
- [ ] Deploy staged z approvals i rollbackiem.
- [ ] Observability pipeline + alerty skonfigurowane.
