---
title: Bezpieczeństwo danych użytkowników
...
Zabezpieczyć dane użytkowników aplikacji (konta, profile, dane behawioralne) w całym cyklu życia i zgodnie z regulacjami prywatności.


## Zakres i granice

- Obejmuje: klasyfikację i minimalizację danych, szyfrowanie w tranzycie/at rest, logowanie z anonimizacją, dostępy (RBAC/ABAC), retencję/usuwanie, udostępnianie partnerom, prawa podmiotów danych.
- Nie obejmuje: szczegółów płatności (PCI) ani polityk marketing automation.
...
1. Klasyfikacja i minimalizacja danych użytkowników.
2. Szyfrowanie i przechowywanie (PII, hasła, tokeny).
3. Dostępy i audyt (role, least privilege, logi).
4. Logowanie/analityka z poszanowaniem prywatności (pseudonimizacja).
5. Retencja/usuwanie i prawa (access/erasure/export).
6. Udostępnianie partnerom (DPA, ograniczenia zakresu).
7. Incydenty i zgłoszenia naruszeń.
...
- [ ] Klasyfikacja/minimalizacja danych wykonana.
- [ ] Szyfrowanie tranzyt/at rest + hasła/hash zgodnie z policy.
- [ ] Dostępy/Audyt logów działają.
- [ ] Retencja/usuwanie i prawa podmiotów wdrożone.
- [ ] Partnerzy/DPA i zakres danych kontrolowane.


## Definicje robocze

- **PII** — dane osobowe pozwalające zidentyfikować osobę.
