...
Zdefiniować ogólne zasady bezpieczeństwa Operational Technology (OT): segmentacja, dostęp, monitoring, patching, zarządzanie zmianą, zgodność.


## Zakres i granice

- Obejmuje: architekturę OT/IT, kontrolę dostępu, hardening urządzeń, zarządzanie zmianą, monitoring, incident response, compliance (ISA/IEC 62443).
- Nie obejmuje: bezpieczeństwa fizycznego/BHP.
...
1. Architektura i segmentacja (strefy/DMZ, firewalle, jump hosty).
2. Dostęp i konta (role, least privilege, logowanie, zdalny dostęp).
3. Hardening i patching (aktualizacje, whitelisty, removable media).
4. Monitoring i IR (logi, IDS/IPS OT, playbooki).
5. Zarządzanie zmianą (okna produkcyjne, testy, rollback).
6. Compliance i audyt (ISA/IEC 62443, rejestry).
...
- [ ] Segmentacja i kontrola dostępu wdrożone.
- [ ] Hardening/patching zaplanowane.
- [ ] Monitoring/IR działa.
- [ ] Zmiany przechodzą przez change mgmt.
- [ ] Compliance śledzone (62443).
