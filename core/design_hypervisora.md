...
Zaprojektować architekturę hypervisora i platformy wirtualizacji: bezpieczeństwo, izolacja, wydajność i zarządzanie.
...
1. Model bezpieczeństwa/izolacji (VM <-> host, VT-d/IOMMU, secure boot).
2. Scheduler i CPU (pinning, NUMA, overcommit policy).
3. Memory management (ballooning, hugepages, dedup/KSM).
4. I/O wirtualizacja (virtio, SR-IOV, storage stack, network stack, MTU).
5. Zarządzanie i API (libvirt/management plane), role/ACL, audyt.
6. HA/live migration i storage consistency.
7. Observability (metrics, logs, tracing), upgrade/patch strategy.
...
- [ ] Izolacja/secure boot/IOMMU opisane.
- [ ] CPU/mem/I/O polityki zdefiniowane.
- [ ] Live migration/HA scenariusze i storage spójność.
- [ ] Audyt/logi i patch strategy przygotowane.
