...
Opisać standardy implementacji w BigQuery: modelowanie, partycjonowanie/klastrowanie, koszty, bezpieczeństwo i operacje, by mieć wydajne i przewidywalne zapytania.


## Zakres i granice

- Obejmuje: schematy i typy, partition/cluster, denormalizacja vs normalizacja, koszt (slots/on-demand), materializacje, storage (tables vs external), bezpieczeństwo (row-level/column-level, IAM), obserwowalność, testy.
- Nie obejmuje: szerokiej architektury danych (osobny dokument) ani narzędzi ETL spoza BigQuery.
...
1. Model danych (schemat, typy, naming, denorm vs norm).
2. Partycjonowanie/klastrowanie (kiedy, jak dobrać kolumny).
3. Optymalizacja kosztu (query stages, slots, materialized views, BI Engine).
4. Bezpieczeństwo (IAM, RLS/CLS, masking), audit logi.
5. Ładowanie i retention (batch/stream, external tables, TTL).
6. Testy/observability (query explain, INFORMATION_SCHEMA, monitoring).
...
- [ ] Schemat/naming i typy ustalone.
- [ ] Partition/cluster dobrane do patternów zapytań.
- [ ] Koszt monitorowany, MV użyte gdy warto.
- [ ] IAM + RLS/CLS skonfigurowane.
- [ ] Testy/monitoring zapytań i ładowań działają.
