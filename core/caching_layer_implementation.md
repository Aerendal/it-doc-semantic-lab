...
Opisać zasady implementacji warstwy cache: co i jak cachować, invalidacja, spójność, monitoring i koszty.


## Zakres i granice

- Obejmuje: typy cache (in-memory/distributed), polityki (TTL/LRU), kluczowanie, idempotencję, dogpile protection, invalidację (time/explicit/event), spójność (write-through/aside), bezpieczeństwo, obserwowalność i budżet kosztowy.
- Nie obejmuje: cache przeglądarki/CDN (są osobno).
...
1. Cele cache i zakres (co cachujemy, a czego nie).
2. Klucze i TTL, polityki eviction, dogpile protection.
3. Strategie spójności (write-through/write-behind/cache-aside).
4. Invalidacja (czasowa, event, manual) i scenariusze.
5. Bezpieczeństwo (dane wrażliwe, authz, izolacja tenantów).
6. Observability i koszt (hit ratio, latency, memory, egress).
...
- [ ] Klucze/TTL i polityka eviction zdefiniowane.
- [ ] Strategia spójności i invalidacji opisana.
- [ ] Ochrona dogpile/thundering herd uwzględniona.
- [ ] Bezpieczeństwo danych w cache zapewnione.
- [ ] Hit ratio/latency/koszt monitorowane.
