...
Zaplanować bezpieczne wycofanie aplikacji/usługi: komunikacja, migracja użytkowników/danych, zamknięcie integracji, minimalizacja ryzyk i kosztów.


## Zakres i granice

- Obejmuje: kryteria wycofania, plan komunikacji, migrację danych/użytkowników, zależności (API, integracje), decommission infra, archiwizację, compliance.
- Nie obejmuje: szczegółowych instrukcji migracji funkcjonalności (to w Migration Runbook) ani strategii replacementu produktu (osobny produktowy dokument).
...
- Wejścia: decyzja biznesowa, lista zależności, kontrakty/SLA, dane użytkowników, wymagania prawne (retencja/RODO), plan alternatywy.
- Wyjścia: harmonogram wycofania, plan komunikacji, plan migracji, runbook decommission, lista ryzyk i mitigacji.
...
- DR/BCP (co z planami awaryjnymi w trakcie wycofania).
...
1. Zakres i powód wycofania.
2. Kryteria i data sunset (freeze, read-only, off).
3. Komunikacja (kogo, kiedy, jak: użytkownicy, partnerzy, wewnętrzni).
4. Migracja użytkowników i danych (ścieżki, wsparcie, fallback).
5. Zależności i integracje (API, webhooki, klienci).
6. Decommission infra (kopie, archiwizacja, wyłączenie, cleanup kosztów).
7. Compliance i audyt (retencja, dowody komunikacji).
...
- [ ] Data i etapy sunset zdefiniowane.
- [ ] Komunikacja zaplanowana i wykonana (dowody).
- [ ] Migracja użytkowników/danych zaplanowana i przetestowana.
- [ ] Zależności/integracje zinwentaryzowane i zamknięte.
- [ ] Decommission infra zrobiony zgodnie z compliance (kopie, logi, koszty).


## Definicje robocze

- **Sunset/Retirement** — proces wygaszania produktu/usługi.
- **Freeze/Read-only/Off** — etapy ograniczania funkcji przed wyłączeniem.
...
- „Harmonogram i komunikacja” **feeds** „Migracja użytkowników/danych”.
- „Zależności” **constrains** „Decommission infra”.
