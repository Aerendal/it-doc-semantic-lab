...
Opisać, jak mierzymy oglądalność/zużycie treści (views, odtworzenia, czas oglądania) i jak używamy tych danych do decyzji produktowych i infrastrukturalnych.


## Zakres i granice

- Obejmuje: definicje zdarzeń (view, play, pause, completion), metryki (unique viewers, watch time, retention, churn), segmentację (device, geo, content type), źródła danych i jakość.
- Nie obejmuje: szczegółowych algorytmów rekomendacji (osobny dokument) ani polityki prywatności (oddzielny).
...
- Wejścia: zdarzenia analityczne, logi odtwarzacza/CDN, dane użytkowników (zgodnie z privacy), wymagania biznesowe.
- Wyjścia: słownik zdarzeń i metryk, schemat danych, raporty/kokpity, zasady próbkowania/retencji.
...
1. Definicje zdarzeń (view/play/pause/complete/error).
2. Metryki kluczowe (unique viewers, watch time, retention, completion rate).
3. Segmentacja (device, geo, plan, content type).
4. Źródła i jakość danych (sampling, duplikaty, late events).
5. Raporty i użycie (dashboardy, alerty na spadki).
6. Retencja/prywatność (jak długo trzymamy, anonimizacja).
...
- [ ] Słownik zdarzeń i metryk spójny.
- [ ] Segmentacja zdefiniowana.
- [ ] Zasady jakości danych (dedup, late events) opisane.
- [ ] Raporty i odbiorcy wskazani.
- [ ] Retencja/prywatność uwzględnione.
...
- „Definicje zdarzeń” **defines_structure** „Metryki i raporty”.
- „Segmentacja” **feeds** „Decyzje produktowe i infra”.
