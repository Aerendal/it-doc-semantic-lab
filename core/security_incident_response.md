...
Zdefiniować procedurę reagowania na incydenty bezpieczeństwa, aby szybko ograniczyć wpływ, przywrócić działanie i zapewnić zgodność/audytowalność.


## Zakres i granice

- Obejmuje: detekcję, klasyfikację, triage, containment, eradication, recovery, komunikację, raportowanie, trigger postmortem i follow-up.
- Poza zakresem: implementacja poprawek produktowych poza zakresem incydentu (oddzielne backlogi).
...
- Detekcja i klasyfikacja incydentu
- Triage i decyzje (C/E/R) + kryteria eskalacji
- Komunikacja i kanały (wewnętrzna/kliencka/regulator)
- Monitorowanie i kryteria zamknięcia
- Raportowanie i postmortem trigger
- Follow-up i action log
...
- Wypełnij klasyfikację, triage i decyzje C/E/R; podlinkuj komunikację i action log; sekcje N/A uzasadnij.
- Dodaj quick-links i checklisty DoR/DoD w `reports/checklist_atomic.jsonl`.
- Po incydencie podlinkuj raport/postmortem i zaktualizuj status działań.
...
- Alerty/logi, zgłoszenia użytkowników.
- Klasyfikacja wstępna, polityki IR, runbooki SOC.
- Lista właścicieli/oncall, kanały komunikacji, dane kontaktowe.


## Wyjścia

- Runbook incydentu z decyzjami C/E/R.
- Plan komunikacji i statusy.
- Action log, raport incydentu i trigger postmortem.
...
- Tabela: krok → właściciel → kanał → SLA → dowód.
- Streszczenie: klasyfikacja, impact, decyzje, next steps.


## Wymagane powiązania

- Polityka IR, runbooki SOC, rejestr ryzyk/incydentów, compliance matrix.
- Kanały komunikacji i szablony (PR/klient/regulator).


## Kryteria DoR

- [ ] Polityka IR i kanały komunikacji dostępne.
- [ ] Oncall/role i kryteria eskalacji zdefiniowane.
- [ ] Szablony komunikatów i action log przygotowane.


## Kryteria DoD

- [ ] Krok po kroku C/E/R opisane; komunikacja i eskalacje zrealizowane.
- [ ] Action log wypełniony; trigger postmortem dodany.
- [ ] Quick-links/checklisty zaktualizowane, metadane bieżące.


## Artefakty do załączenia

- Action log i timeline incydentu.
- Komunikaty/ogłoszenia (wew./zewn.).
- Raport incydentu/postmortem link.


## Walidacja / testy

- Dry-run scenariuszy incydentów; test kanałów komunikacji.
- Sprawdzenie zgodności z regulacjami (np. SLA powiadomień).


## Metryki monitorowane

- MTTA/MTTR; SLA powiadomień/regulator.
- % incydentów z kompletnym action logiem/postmortem.
- Liczba FP/TP i skuteczność triage.


## Utrzymanie i aktualizacje

- Przegląd co sprint/msc lub po każdym incydencie/postmortem.
- Aktualizuj role, kanały, szablony i kryteria eskalacji.


## Zakończenie

Po spełnieniu DoD podlinkuj artefakty, odhacz checklisty w `reports/checklist_atomic.jsonl` i poinformuj właścicieli o follow-up.
