...
Dostarczyć wytyczne implementacyjne dla aplikacji Flutter.
...
1. Architektura: wybór state management (Bloc/Provider/Riverpod), modularność, DI.
2. UI/UX: theming, responsywność, dostępność, internationalization.
3. Networking i dane: REST/gRPC, caching, offline, serializacja, error handling.
4. Build i CI/CD: flavoring, env, testy (unit/widget/integration), signing.
5. Wydajność: lazy loading, jank profiling, obrazki, isolates.
6. Bezpieczeństwo: storage tajemnic, TLS pinning, jailbreak/root detection.
...
- [ ] Wybrano i udokumentowano wzorzec state management.
- [ ] Testy unit/widget/integration są w pipeline; buildy są podpisane.
- [ ] Offline/cache i obsługa błędów zaimplementowane.
- [ ] Profil wydajności zaakceptowany; kontrole bezpieczeństwa wdrożone.
