---
title: Dokumentacja API kwantowych
...
Opisać API udostępniające obliczenia/zasoby kwantowe (QPU/emu): kontrakty, wersje, bezpieczeństwo i limity.
...
1. Zakres i modele (jobs/circuits/annealing), endpointy, przykłady request/response.
2. Auth/security: mTLS/OAuth, podpisy, quotas, isolation.
3. Parametry jobów (shots, backend, precision), validation, idempotencja.
4. Limity i SLO: queue times, max depth/qubits, timeouts, rate limits.
5. Wyniki i formaty: pomiary, metadata, error metrics, logs.
6. Observability: request/job IDs, statusy, błędy, retry guidance.
7. Versioning/deprecacja i komunikacja zmian.
...
- [ ] Endpointy/przykłady aktualne.
- [ ] Auth/limity i format wyników opisane.
- [ ] SLO/queue/timeouty i błędy zdefiniowane.
- [ ] Versioning/deprecacja zakomunikowane.
