...
Opisać referencyjną architekturę lakehouse łączącą zalety data lake i data warehouse: warstwy danych, governance i wydajność.
...
1. Warstwy danych: bronze/silver/gold, formaty (Parquet/Delta/Iceberg), katalog/metastore.
2. Ingest i przetwarzanie: batch/stream, ACID na jeziorze, CDC, joby ETL/ELT.
3. Dostęp i modele: SQL/BI, ML/feature store, data products, caching/serving.
4. Governance i bezpieczeństwo: katalog schematów, kontrola dostępu row/column, maskowanie, linage, audit.
5. Wydajność i koszt: partitioning, Z-order, compaction/vacuum, storage classes.
6. Operacje: monitoring, quality checks (DQ), backup/DR, wersjonowanie danych.
...
- [ ] Warstwy i formaty lakehouse zdefiniowane (ACID na jeziorze).
- [ ] Ingest/ETL/CDC opisane z podziałem batch/stream.
- [ ] Dostęp (SQL/BI/ML) i kontrola row/column-level wdrożone.
- [ ] Partitioning/compaction/monitoring i DQ uruchomione; governance zapewnione.
