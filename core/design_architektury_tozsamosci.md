---
title: Design architektury tożsamości
...
aligned_by: codex
---
# Design architektury tożsamości
...
Zaprojektować architekturę tożsamości (IAM): IdP/SSO, provisioning, autoryzacja, audyt i zgodność.
...
1. IdP/SSO i federacja (SAML/OIDC), multi-tenant/B2B/B2C.
2. Identity store i źródła prawdy (HR/CRM), SCIM/provisioning/deprovisioning.
3. AuthN: MFA, risk-based, session/refresh/token policy.
4. AuthZ: RBAC/ABAC, policy engine, SoD, least privilege.
5. Bezpieczeństwo: klucze/secrets, mTLS, anti-abuse, rate limits.
6. Audyt i zgodność (logi, SIEM, recertyfikacje, access reviews).
7. Developer experience: SDK, self-service, consent/PII.
...
- [ ] IdP/SSO/federacja i provisioning opisane.
- [ ] MFA/token/session policy zdefiniowane.
- [ ] RBAC/ABAC/SoD i policy engine wdrożone.
- [ ] Audyt/logi/re-cert i compliance działają.
