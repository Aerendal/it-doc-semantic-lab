...
Przewodnik tworzenia i utrzymania jobów Flink: źródła, state, checkpointing, skalowanie i observability.
...
1. Źródła/sinki i formaty (Kafka/files/DB), watermarking/timestamps.
2. State i checkpointing/savepoints (interval, backends, retention).
3. Semantyki: exactly/at-least-once, idempotencja, reprocessing.
4. Skalowanie: parallelism, rescaling, backpressure, resources.
5. Observability: metrics (lag, backpressure, checkpoint duration), logs, alerts.
6. Bezpieczeństwo: auth do źródeł/sinków, PII, encryption.
7. Deploy/upgrade: per-env config, savepoint migration, rollback.
...
- [ ] Watermark/timestamps i semantics zdefiniowane.
- [ ] Checkpoint/savepoint i backend ustawione.
- [ ] Lag/backpressure/alerts monitorowane.
- [ ] Rescaling/upgrade z savepoint/rollback opisane.
