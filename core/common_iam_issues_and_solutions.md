---
title: Common IAM Issues and Solutions
...
Lista typowych problemów w IAM (tożsamość/dostępy) i szybkie sposoby ich rozwiązania.
...
- [ ] Least privilege i SoD zaimplementowane.
- [ ] MFA dla kont uprzywilejowanych.
- [ ] Rotacja kluczy/secrets i JIT/PAM.
- [ ] Offboarding automatyczny, recertyfikacje okresowe.
- [ ] Audit/logi IAM w SIEM, alerty na zmiany uprawnień.
...
- **Brak least privilege**: zbyt szerokie role → przegląd uprawnień, SoD, role per job, recertyfikacje.
- **Shadow admins/klucze statyczne**: stare klucze/API keys → rotacja, secret store, krótkie TTL, JIT access/PAM.
- **Brak MFA**: konta uprzywilejowane bez MFA → wymuś MFA, zwłaszcza dla admin/remote.
- **Zombie accounts**: konta po offboard → automatyczny offboarding z HR feed, cykliczny access review.
- **Brak audytu**: logi niedostępne → włącz audit/CloudTrail/AD logs, centralne SIEM.
- **SSO misconfig**: błędne claimy/clock skew → validacja claimów, synchronizacja czasu, testy federacji.
- **Identyfikacja użytkownika**: duplikaty/aliasy → unikalny ID, SCIM, MFA re-bind.
