...
Zdefiniować bazowy poziom wydajności (Performance Baseline) projektu obejmujący zakres, harmonogram i budżet — jako punkt odniesienia do pomiaru postępu zgodnie z PMBOK 7 (Performance Measurement Baseline). Baseline umożliwia stosowanie Earned Value Management (EVM), identyfikację odchyleń (SV, CV, SPI, CPI) i zarządzanie zmianami zakresu przez Integrated Change Control.
