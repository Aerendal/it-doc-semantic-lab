...
## Cel dokumentu
Zapewnić spójną i terminową komunikację podczas incydentów do wewnętrznych i zewnętrznych interesariuszy, aby ograniczyć niepewność i przyspieszyć decyzje.

## Zakres i granice
Obejmuje: role komunikacyjne, kanały, częstotliwość update’ów, szablony komunikatów, listy adresatów, zasady informacji wrażliwych, koordynację z Incident Commanderem. Nie obejmuje: technicznych kroków naprawczych (runbooki).

## Wejścia i wyjścia
- Wejścia: Incident Response Plan, klasyfikacja/severity, lista interesariuszy, polityki PR/legal, status techniczny z IR.
- Wyjścia: harmonogram i kanały komunikacji, szablony komunikatów (wew./zew.), dziennik komunikacji, lista odbiorców i odpowiedzialni.
...
- Zakres stosowania i zasady (tone, no-blame, minimal disclosure)
- Role i odpowiedzialności (comms lead, IC, PR/legal, support)
- Klasyfikacja incydentu → częstotliwość i kanały komunikacji
- Listy odbiorców (wew./zew.) i preferowane kanały
- Szablony komunikatów (initial, updates, resolved, RCA/PIR zapowiedź)
- Zasady informacji wrażliwych (PII, regulatorzy, NDA)
- Dziennik komunikacji i audyt
