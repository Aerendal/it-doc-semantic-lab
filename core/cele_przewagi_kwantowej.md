...
Określić, jakie mierzalne korzyści (czas/ jakość rozwiązania/koszt) ma przynieść użycie obliczeń kwantowych względem klasycznych rozwiązań.


## Zakres i granice

- Obejmuje: use case’y kandydackie, metryki przewagi (czas, dokładność, koszt), benchmarki klasyczne vs kwantowe, kryteria akceptacji, ryzyka i koszty QPU.
- Nie obejmuje: projektowania algorytmów kwantowych (osobny dokument).
...
1. Use case’y i metryki sukcesu (czas/ jakość / koszt).
2. Benchmark baseline klasyczny (metoda, wynik, koszt).
3. Plan eksperymentów kwantowych (framework, QPU/emu, dane).
4. Kryteria przewagi/akceptacji (np. ≥X% poprawy lub ≤Y kosztu).
5. Ryzyka i koszty (czas QPU, kolejki, dostępność).
6. Raportowanie wyników i decyzja „go/no-go”.
...
- [ ] Metryki przewagi zdefiniowane.
- [ ] Baseline klasyczny zmierzony.
- [ ] Plan eksperymentów kwantowych gotowy.
- [ ] Kryteria akceptacji ustalone.
- [ ] Koszty/ryzyka QPU uwzględnione.
