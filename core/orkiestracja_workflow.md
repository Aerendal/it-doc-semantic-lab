...
Zaplanować i wdrożyć orkiestrację workflow (batch/stream/hybrydowe) z niezawodnością, obserwowalnością i zarządzaniem zmianami.
...
1. Zakres: typy zadań, zależności, środowiska, SLA.
2. Narzędzia: orkiestrator (Airflow/Prefect/Argo/etc.), DAG, storage stanu, kolejki.
3. Niezawodność: retry/backoff, idempotencja, checkpointy, timeouts, compensation.
4. Observability: logi, metryki, tracing, SLA monitoring, alerty, DLQ.
5. Bezpieczeństwo: sekrety, uprawnienia do zasobów, izolacja tenantów, audyt.
6. Operacje: deployment pipeline, wersjonowanie DAG, change management, testy, runbooki.
...
- [ ] SLA i zależności workflow zmapowane.
- [ ] Retry/idempotencja/checkpointy i timeouts skonfigurowane.
- [ ] Monitoring logów/metryk/traces + alerty i DLQ działa.
- [ ] Sekrety/uprawnienia i change management zaadresowane; DAG wersjonowane.
