...
Wdrożyć bezpieczne uwierzytelnianie/autoryzację oparte o OAuth 2.1/OIDC z tokenami JWT.
...
1. Role systemów: Authorization Server, Resource Servers, klienci (public/confidential).
2. Granty i przepływy: auth code + PKCE, client credentials, device; kiedy stosować.
3. Parametry JWT: algorytmy, rotacja kluczy (JWKS), TTL, aud/iss, minimalne scope.
4. Bezpieczeństwo: MFA, replay protection, token binding, refresh token hygiene.
5. Integracje: SSO, gateway, mobilne/SPA, legacy; migracja i fallback.
6. Observability: logowanie, audyt, metryki błędów i latencji; procedury incydentów.
...
- [ ] Wybrane właściwe granty i konfiguracje dla typów klientów.
- [ ] JWT mają poprawne parametry (alg, kid, aud, exp) i rotację kluczy.
- [ ] Zaimplementowano MFA, ochronę przed replay i bezpieczne refresh tokens.
- [ ] Monitoring i audyt przepływów OAuth/OIDC włączony.
