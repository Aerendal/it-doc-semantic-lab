...
## Cel dokumentu
Opisać bezpieczne wycofanie firmware’u do poprzedniej wersji na flocie urządzeń, z minimalnym ryzykiem dla sprzętu i użytkowników.

## Zakres i granice
Obejmuje: kryteria wyzwolenia rollback, przygotowanie pakietów FW, weryfikację zgodności HW, kroki wdrożeniowe (partycje/dual-bank/OTA), walidację po rollbacku, komunikację, bezpieczeństwo danych/kluczy. Nie obejmuje: projektowania firmware’u ani pipeline build.

## Wejścia i wyjścia
- Wejścia: Deployment/Release Plan FW, poprzedni stabilny obraz, lista urządzeń/partycji, mechanizmy OTA, metryki zdrowia, wymagania bezpieczeństwa.
- Wyjścia: przywrócony FW, potwierdzone zdrowie urządzeń, log działań, raport incydentu/rollback, komunikaty do klientów.
...
- Software Requirements (REQUIRES)
- Software Requirements (REQUIRES)
...
- Kryteria wyzwolenia rollback (telemetria, błędy, bezpieczeństwo)
- Przygotowanie: obrazy FW, podpisy, kompatybilność HW/bootloader
- Kroki rollback (OTA/USB/JTAG; sekwencja, batchowanie, throttle)
- Walidacja po rollbacku (telemetria, health checks, bezpieczeństwo kluczy)
- Komunikacja do użytkowników/partnerów i aktualizacja statusu
- Eskalacja, log działań i plan dalszych poprawek
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Software Requirements', 'type': 'REQUIRES'}
- Zależność: {'depends_on': 'Software Requirements', 'type': 'REQUIRES'}
