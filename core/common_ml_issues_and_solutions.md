---
title: Common ML Issues and Solutions
...
Zebrać typowe problemy w cyklu ML i szybkie remedia.
...
- [ ] Brak leakage (kontrole czasowe).
- [ ] Balans klas i właściwa metryka.
- [ ] Drift monitoring i plan retrain.
- [ ] Optymalizacja inferencji (pruning/quantization) tam gdzie potrzebne.
- [ ] Parity train/serve i tests kontraktowe.
...
- **Data leakage**: cechy z przyszłości → split czasowy, leak check, sanity.
- **Imbalanced data**: klasy rzadkie → class weights, focal loss, sampling, odpowiednia metryka.
- **Drift**: dane się zmieniły → monitoring driftu, alerty, retrain schedule.
- **Overfitting**: zbyt skomplikowany model → regularizacja, dropout, prostszy model, cross-val.
- **Zbyt wolna inferencja**: duży model → pruning/quantization/distillation, batching, cache.
- **Explainability**: brak interpretacji → SHAP/perm importance, model card.
- **Deployment mismatch**: różnice feature pipeline → feature store, test parity, contract tests.
