...
## Cel dokumentu
Krótka definicja: QA Test Plan opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
- Architektura systemu (INFORMED_BY) — aby wiedzieć, które integracje i ścieżki użytkownika są krytyczne.
- Wymagania biznesowe i NFR (INFORMED_BY) — zakres funkcjonalny i niefunkcjonalny.
- Plan release / roadmap (INFORMED_BY) — daty, okna testowe, freeze.
- Dane testowe / PIA (INFORMED_BY) — wymagania prywatności i legalności danych.
...
- Zakres i cele: co jest w/out; kryteria akceptacji releasu.
- Strategia testów: poziomy (unit/API/UI), rodzaje (funkcjonalne, regresja, bezpieczeństwo, wydajność, użyteczność, dostępność), depth vs breadth.
- Matryca pokrycia: wymagania → scenariusze → przypadki testowe → automaty/przypadki manualne.
- Harmonogram i kamienie milowe: przygotowanie danych, smoke, regresja, UAT, performance window, freeze, go/no-go.
- Role i RACI: QA Lead, QA Engineer, Developer (pair testing), Product, Security, Ops, DPO.
- Środowiska: plan środowisk (dev/test/stage/pre-prod), wersjonowanie, zarządzanie konfiguracją, seedy danych.
- Dane testowe i prywatność: generowanie/anonymizacja, zarządzanie sekretami, dostępność, rotacja.
- Narzędzia i automatyzacja: pipeline CI/CD, runnerzy, repo testów, raportowanie (dashboards), flaki i retry polityki.
- Zarządzanie defektami: klasyfikacja (P0–P3), SLA napraw, eskalacja, akceptacja ryzyka (waivers).
- Kryteria wejścia/wyjścia (DoR/DoD): warunki startu cyklu, warunki zakończenia i go-live.
- Ryzyka i plan mitigacji: ryzyka produktowe, techniczne, ludzi (rotacja), środowiskowe.
- Komunikacja: rytuały (triage, standup QA, sign-off), odbiorcy raportów, kanały eskalacji.

## Wymagane rozwinięcia
- Strategia testów rozwija wymagania z: QA Strategy Document, Security Test Plan, Performance Test Plan.
- Dane testowe rozwijają: Data Management Plan, Privacy Impact Assessment.
- Narzędzia/automatyzacja rozwija: CI/CD Pipeline Design, Test Automation Framework Guide.

## Wymagane streszczenia
- Streszczenie architektury i przepływów użytkownika (dla mapowania ścieżek krytycznych).
- Streszczenie harmonogramu releasu z Release Plan (daty, freeze, okna testowe).
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Code Architecture Document', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Gameplay Implementation', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Code Architecture Document', 'type': 'INFORMED_BY'}
...
- DoR cyklu testowego:
  - [ ] Zakres i kryteria akceptacji uzgodnione (Product/Tech/QA).
  - [ ] Środowiska i dane testowe dostępne, dane zgodne z PIA/RODO.
  - [ ] Matryca pokrycia wymagań gotowa (traceability startowe).
  - [ ] Narzędzia/CI/CD skonfigurowane, raportowanie działa.
  - [ ] Role i eskalacje opisane.
- DoD cyklu testowego:
  - [ ] Wszystkie testy w zakresie wykonane; defekty P0/P1=0 lub zaakceptowane waivery.
  - [ ] Raport z metryk (coverage, DRE, flaki, MTTR defektów) opublikowany.
  - [ ] Regression/Smoke green; performance/security wyniki w normie lub z planem działań.
  - [ ] Sign-off i decyzja go/no-go udokumentowana.
