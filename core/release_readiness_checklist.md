...
## Cel dokumentu
Zweryfikować, czy release jest gotowy do wdrożenia: wymagania jakościowe, bezpieczeństwo, compliance, monitoring, wsparcie, komunikacja, rollback. Checklistę odhacza się przed Go/No-Go.

## Zakres i granice
Obejmuje: testy funkcjonalne i NFR, sign-off właścicieli komponentów, audyty bezpieczeństwa/privacy, status migracji danych, monitoring/alerty, runbooki i on-call, komunikację, plan rollback. Nie obejmuje: szczegółów kroków deploymentu (Deployment Plan).

## Wejścia i wyjścia
- Wejścia: Test Execution Report, Security/Privacy sign-off, Release Plan, Deployment Plan, lista znanych defektów, Risk Register.
- Wyjścia: wypełniona checklista z podpisami/ownerami, decyzja Go/No-Go, lista blokujących gapów jeśli No-Go.
...
- Test Execution Report (REFERENCES)
- Test Execution Report (REFERENCES)
...
- Testy i jakość (func, regression, perf, sec, a11y)
- Migracje danych i kompatybilność wsteczna
- Monitoring/observability (metryki, alerty, dashboardy)
- Runbooki/on-call i wsparcie L1/L2/L3
- Bezpieczeństwo i zgodność (SAST/DAST, privacy, licencje)
- Komunikacja release (wew./zew.), materiały dla CS/Support
- Plan rollback i weryfikacja backupów
- Podpisy/ownerzy i decyzja Go/No-Go
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Test Execution Report', 'type': 'REFERENCES'}
- Zależność: {'depends_on': 'Test Execution Report', 'type': 'REFERENCES'}
