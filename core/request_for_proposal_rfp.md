...
Request for Proposal (RFP) — szablon dokumentu IT.

Dokumentuje wymagania wobec dostawcy, warunki kontraktowe lub wyniki oceny ofert. Zawiera: kryteria oceny (techniczne, finansowe, compliance), wymagania SLA, klauzule bezpieczeństwa i warunki zakończenia współpracy.

> Dokument satelitarny do: "Request for Proposal" - rozszerza lub uszczegolawia jego tresc.
...
- **Wejścia** (co musi być dostępne przed wypełnieniem): Wymagania biznesowe i techniczne, polityki zarządzania dostawcami (ISO 27001 A.15), budżet, harmonogram wyboru, lista kandydatów.
- **Wyjścia** (co dokument wytwarza jako rezultat): Zatwierdzona umowa lub ocena dostawcy z uzasadnieniem, SLA/OLA z dostawcą, plan onboardingu dostawcy, rejestr dostawców.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): Rejestr dostawców (vendor register), plany audytów dostawców, plany zarządzania ryzykiem łańcucha dostaw, raporty compliance.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Polityka zarządzania dostawcami, wymagania bezpieczeństwa informacji, standardy regulacyjne (GDPR, NIS2), budżet IT.
