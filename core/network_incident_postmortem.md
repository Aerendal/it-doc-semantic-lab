...
## Cel dokumentu
Przeanalizować incydent sieciowy (łączność, latency, packet loss, DNS/LB), zidentyfikować przyczyny i zaplanować działania zapobiegawcze.

## Zakres i granice
Obejmuje: timeline, wpływ na usługi/regiony, warstwę L3/L4/L7, DNS/LB, BGP/peering, zmiany konfiguracji, CAPA. Nie obejmuje: pełnej architektury sieci (ta jest w deployment/arch docs).

## Wejścia i wyjścia
- Wejścia: NetFlow/pcap, health LB, DNS metrics, traceroute, zmiany ACL/route, alerty SLO, ruch/peaks.
- Wyjścia: postmortem sieciowy, działania (routing/LB policies, DNS TTL, limits), testy syntetyczne/chaos, aktualizacje runbooków/monitoringu.
...
- Streszczenie i wpływ (regiony/usługi, RTO/RPO)
- Timeline (zdarzenia, zmiany, failovers)
- Root cause + contributing (BGP/LB/DNS/ACL, capacity, config drift)
- Diagnostyka: co zadziałało, co zawiodło (synthetics, alerts)
- CAPA: polityki LB/DNS, peering, QoS/rate limits, redundancja/HA
- Testy regresyjne/syntetyczne, monitoring aktualizacje
