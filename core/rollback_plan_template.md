...
## Cel dokumentu
Dać wzorzec do budowy planów rollbacku: kiedy uruchamiać, jakie kroki, walidacje, komunikacja i logowanie działań.

## Zakres i granice
Obejmuje: warunki wyzwolenia, kroki techniczne, walidacje, komunikację, role, log działań. Nie obejmuje: detali specyficznych dla danej usługi (uzupełnia je właściciel).

## Wejścia i wyjścia
- Wejścia: Release/Deployment Plan, wersje artefaktów, metryki SLO/SLA, ryzyka.
- Wyjścia: uzupełniony plan rollbacku gotowy do użycia, checklisty i log wykonania.
...
- Artefakty i pre-req (wersje, backupy, dane)
- Kroki rollback (numerowane; dane/config/app/API/DB/CDN)
- Walidacja po rollbacku (smoke, dane, monitoring)
- Komunikacja i eskalacja
- Log działań i ownerzy
