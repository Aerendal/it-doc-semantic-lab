...
## Cel dokumentu
Opisać, jak szybko i bezpiecznie wycofać wdrożenie do poprzedniej stabilnej wersji, minimalizując wpływ na użytkowników i dane.

## Zakres i granice
Obejmuje: warunki aktywacji rollback, warianty (całość vs częściowy/feature-flag), kroki techniczne, walidację po rollbacku, komunikację, eskalacje. Nie obejmuje: pełnego planu deploy (Deployment Plan) ani postmortemów.

## Wejścia i wyjścia
- Wejścia: Deployment/Release Plan, wersja referencyjna, artefakty poprzedniej wersji, backupy/migracje, metryki/alerty wskazujące na degradację.
- Wyjścia: przywrócona stabilna wersja, raport z rollbacku (czas, przyczyna, kroki), aktualizacja statusu releasu i komunikacja do interesariuszy.
...
- Production Deployment Guide (REQUIRES)
...
- Warunki wyzwolenia rollback (metryki, błędy, SLA/SLO, eskalacje)
- Przygotowanie: artefakty poprzedniej wersji, backup/restore danych, feature flags
- Kroki rollbacku (numerowane; DB/schema, app, config, cache, CDN)
- Walidacja po rollbacku (smoke, zdrowie, dane)
- Komunikacja i aktualizacja statusu
- Eskalacja i przekazanie do PIR/postmortem
- Checklist wykonania i log działań
- [Sekcja 3]
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Production Deployment Guide', 'type': 'REQUIRES'}
