---
title: Architecture Compliance Testing
...
## Cel dokumentu
Zaplanować i wykonać testy potwierdzające zgodność architektury z guardrails/standardami (security, availability, data, IAM, network, privacy, PCI/SOC2/ISO), wykryć odchylenia i dostarczyć dowody dla arch review/audytu/go-live.

## Zakres i granice
- Obejmuje: testy projektowe i techniczne (IaC policy-as-code, arch review cases, threat model validation, performance/resilience, security controls, data/privacy), kryteria zaliczenia, dowody, powiązania z releasami/go-live, wyjątki/waivery.
- Poza zakresem: szczegółowe runbooki operacyjne (są w playbookach) i pełne pen-testy (linki tylko).

## Wejścia i wyjścia
- Wejścia: Arch Guidelines/Guardrails, Security Baseline, Cloud Best Practices, ADR, Threat Models, wymagania PCI/SOC2/ISO/Privacy, IaC/policy scan rules, environment diagrams, DR/BCP wymagania.
- Wyjścia: plan i wyniki testów compliance architektury, lista odchyleń/waiverów, dowody (raporty skanów/logi/testy), rekomendacje i warunki go/conditional/no-go.
...
- Architecture Compliance Monitoring/Dashboard.
- Threat Models / ADR / Security Baseline / Cloud Best Practices.
- Release/Change Plan (gating), Test Strategy/Security Testing Plan.
...
- Zakres testów: domeny (security/privacy/IAM/network/data/availability/cost), systemy/komponenty, środowiska.
- Metody i narzędzia:
  - IaC/Policy-as-Code (OPA/Conftest/TF validate).
  - Arch review cases i checklisty (guardrails).
  - Threat Model validation (STRIDE/OWASP), attack surface checks.
  - Security scans (SAST/DAST/IAST) tam gdzie wymagane architektonicznie.
  - Resilience/performance: chaos/DR drills, capacity, latency/SLO.
  - Data/Privacy: szyfrowanie, klucze, lokalizacja/transfery, PIAs.
- Kryteria zaliczenia: progi, blokery, warunki conditional (kompensacje).
- Plan wykonania: kto, kiedy, środowiska, prereqs, okna testów.
- Wyniki i dowody: raporty, logi, zrzuty, linki; rejestr odchyleń i waiverów z sunset.
- Decyzje go/conditional/no-go: rekomendacje i warunki.

## Wymagane rozwinięcia
- Guardrails/Security Baseline/Cloud Best Practices → sekcje kontroli.
- Threat Models → scenariusze testów.
- DR/BCP → scenariusze DR drills.
- Privacy/PIA → testy transferów, szyfrowania, retention.

## Wymagane streszczenia
- Streszczenie wyników testów, blokery i wygasające waivery.
- Streszczenie warunków go/conditional/no-go dla releasu.
...
## Checklisty jakości
- [ ] Zakres testów i domen zdefiniowany; powiązania do guardrails/Baseline/Threat Models wskazane.
- [ ] Metody/narzędzia dobrane do domen; kryteria zaliczenia i blokery opisane.
- [ ] Plan (kto/kiedy/środowiska) określony; prereqs spełnione.
- [ ] Wyniki mają dowody; odchylenia/waivery z sunset i kompensacjami.
- [ ] Decyzje go/conditional/no-go i warunki wpisane; linki do Release/Change/Test.
- [ ] Kryteria DoR/DoD poniżej spełnione; metadane aktualne.

## Kryteria ukończenia (DoD)
- Testy wykonane zgodnie z planem; dowody zebrane.
- Odchylenia/waivery zapisane z sunset/kompensacjami; rekomendacje i decyzje udokumentowane.
- Linki do arch dashboards/monitoring, Threat Models, Release/Change/Test działają.
- Metadane aktualne; sekcje N/A uzasadnione.

## Kryteria wejścia (DoR)
- Guardrails/Baseline/Threat Models dostępne; scope systemów ustalony.
- Środowiska i narzędzia testowe gotowe; okna testów uzgodnione.
- Uzgodnione progi zaliczenia z Arch Board/Security.
