...
## Cel dokumentu
Ustalić harmonogram realizacji projektu: kamienie milowe, iteracje, zależności i ścieżkę krytyczną, aby zsynchronizować zespoły i zarządzać ryzykiem czasowym.

## Zakres i granice
Obejmuje: kamienie milowe, iteracje/sprinty, zależności między pakietami prac, ścieżkę krytyczną, bufory, plan releasów, kalendarz ryzyk czasowych. Nie obejmuje: szczegółowych tasków zespołów (backlog).

## Wejścia i wyjścia
- Wejścia: WBS, Project Charter (zakres/KPI), estymaty, dostępność zespołów, zależności zewnętrzne, kalendarz świąt/maintenance.
- Wyjścia: harmonogram z datami i zależnościami, wizualizacja (Gantt/burndown), lista kamieni z kryteriami, plan releasów, punkty kontroli/status cadence.
...
- Konsumuje: WBS, estymaty, dostępność zasobów, ryzyka.
- Dostarcza do: Plan release, Plan testów, Plan komunikacji/statusów, Change Management Plan.

- Zależności pakietów → odzwierciedlenie w ścieżce krytycznej.
- Kamienie milowe → powiązanie z raportowaniem i kryteriami releasu.
- Bufory → powiązanie z ryzykami i planem eskalacji.
...
- Założenia i kalendarz (święta, blackouty, okna maintenance)
- Kamienie milowe i kryteria akceptacji
- Iteracje/sprinty i zakres per iteracja
- Zależności i ścieżka krytyczna (diagram/y)
- Bufory czasowe i zasady ich użycia
- Plan releasów (okna, freeze, rollback)
- Status cadence i raportowanie postępu
- Ryzyka harmonogramowe i eskalacja
