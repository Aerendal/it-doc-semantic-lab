---
title: Design alokacji zasobów VM
...
Zaprojektować zasady alokacji zasobów dla maszyn wirtualnych: sizing, overcommit, polityki i monitoring.
...
1. Klasy VM i profile (CPU/RAM/IO), standardy sizingu.
2. Overcommit i limity (CPU/RAM), NUMA/pinning jeśli potrzebne.
3. Storage/IO (typ dysku, IOPS, cache), sieć (vNIC, MTU, QoS).
4. Polityki placement/affinity/anti‑affinity, HA.
5. Monitoring i alerty (CPU ready, memory pressure, IO latency).
6. Proces wnioskowania/zmian (request/approval, lifecycle).
...
- [ ] Profile VM zdefiniowane.
- [ ] Overcommit/NUMA/pinning opisane.
- [ ] Storage/IO i sieć parametry ustalone.
- [ ] Placement/HA polityki opisane.
- [ ] Monitoring CPU ready/mem/IO z alertami.
