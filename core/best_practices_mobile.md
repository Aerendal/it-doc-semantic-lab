...
Zebrać kluczowe dobre praktyki dla aplikacji mobilnych (Android/iOS): architektura, wydajność, UX, bezpieczeństwo, testy i release.


## Zakres i granice

- Obejmuje: architekturę (MVVM/MVI/Clean), DI, zarządzanie stanem, sieć i cache, offline, wydajność (start time, jank, ANR), bezpieczeństwo (cert pinning, storage), analitykę i logi, testy (unit/UI/E2E), release i feature flags.
- Nie obejmuje: szczegółów UI/UX (osobny design guide) ani pełnej specyfikacji API.
...
- Wejścia: wymagania produktowe, wytyczne platform, polityka bezpieczeństwa, metryki jakości.
- Wyjścia: lista praktyk do stosowania w projektach, checklisty QA/release, odniesienia do szablonów/komponentów.
...
1. Architektura i moduły (wzorzec, DI, separacja).
2. Sieć i dane (retry, cache, offline, wersjonowanie API).
3. Wydajność (app start, jank, pamięć, battery).
4. Bezpieczeństwo (cert pinning, storage szyfrowany, PII).
5. Observability (crash/ANR, RUM, logi).
6. Testy (unit/UI/E2E, contract, snapshot).
7. Release i feature flags (beta, staged rollout).
...
- [ ] Wzorzec architektury i DI użyty.
- [ ] Sieć: retry/backoff, cache, offline.
- [ ] Wydajność: start/jank/zużycie mocy monitorowane.
- [ ] Bezpieczeństwo: cert pinning, szyfrowanie storage, PII zasłonięte.
- [ ] Testy i release (staged) wykonane.


## Definicje robocze

- **Jank/ANR** — zacięcia/UI nie odpowiada.
...
- „Architektura i stan” **defines_structure** „Testy i release”.
- „Bezpieczeństwo” **constrains** „Storage/sieć”.
