...
Opisać architekturę i decyzje projektowe dla backupu/odtwarzania, tak aby zapewnić wymagane RPO/RTO, zgodność regulacyjną i spójność z DRP/BCP oraz runbookami usług.


## Zakres i granice

- Zakres: wybór typów backupów (full/inc/diff/snapshot/log), topologia (on-prem/cloud/offsite), retencja, szyfrowanie/KMS, lokalizacja/separacja danych, monitoring, projekt procedur restore/testów, integracja z DRP/BCP.
- Poza zakresem: runbooki operacyjne per aplikacja, bezpieczeństwo fizyczne.
...
- W sekcji 1 zbierz wymagania i założenia; w sekcji 2 zaprojektuj topologię i polityki.
- Dodaj procedury high‑level, monitoring i testy (sekcje 3–5), zgodność i role (6–7).
- Udokumentuj ryzyka/decision log (sekcja 8) i plan przeglądów (sekcja 9); zaktualizuj quick links i checklisty w `reports/checklist_atomic.jsonl`.
...
- RPO/RTO z BIA/BCP/DRP, inwentarz systemów/danych, profile obciążenia, wymagania regulatorów/klientów (retencja/lokalizacja), budżet/storage, dostępne technologie (backup, replication, KMS/HSM).


## Wyjścia

- Projekt architektury backup/restore (diagramy, decyzje technologiczne), macierz RPO/RTO, polityki retencji/szyfrowania/lokalizacji, plan testów, matryca odpowiedzialności, quick links w `linkage_index.jsonl`.
...
- Macierz RPO/RTO z uzasadnieniem biznesowym; decyzje techniczne (snapshot vs log shipping vs replika) z trade‑off.
- Plan testów odtwarzania i metryki; wymagania regulatorów (retencja/lokalizacja) i sposób spełnienia.
- Streszczenie głównych ryzyk (koszt vs RPO/RTO, lokalizacja danych) i action items.


## Wymagane powiązania

- Polityka kluczy KMS/HSM, SLA dostawców storage/replication, rejestr wymagań regulatorów, runbooki usług, monitoring/alerty.


## Kryteria DoR (Definition of Ready)

- [ ] RPO/RTO i klasyfikacja danych/usług dostępne; inwentarz systemów zebrany.
- [ ] Technologie backup/replication wybrane; wymagania regulatorów/lokalizacji znane.


## Kryteria DoD (Definition of Done)

- [ ] Sekcje 1–9 uzupełnione; architektura spójna z DRP/BCP; quick links zaktualizowane.
- [ ] Plan testów, monitoring i role opisane; decyzje/ryzyka udokumentowane; checklisty DoR/DoD odhaczone.


## Artefakty do załączenia

- Diagramy architektury, macierz RPO/RTO, polityki retencji/szyfrowania, decision log, plan testów, wymagania regulatorów.


## Walidacja / testy

- Przegląd techniczny z właścicielami usług i bezpieczeństwem danych.
- Sprawdzenie zgodności z wymaganiami regulatorów (retencja/lokalizacja).


## Metryki monitorowane

- Pokrycie RPO/RTO, koszt storage vs budżet, sukces testów restore, czas dostępu do kluczy w DR, liczba otwartych action items z designu.


## Utrzymanie i aktualizacje

- Przegląd po każdej dużej zmianie architektury/usługi lub kwartalnie; rejestr zmian w `reports/change_log.jsonl`.
- Aktualizacja quick links po każdej iteracji designu/testów.
