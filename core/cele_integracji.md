...
Określić cele i KPI dla integracji między systemami: niezawodność, spójność, czas dostarczenia danych i koszt utrzymania.


## Zakres i granice

- Obejmuje: SLA/SLO integracji, opóźnienie/throughput, spójność danych, odporność (retry/idempotencja), błędy i alerty, koszt ruchu/egress.
- Nie obejmuje: szczegółów pojedynczych API (są w specyfikacjach) ani wyboru stylu integracji (opisany w best practices integracji).
...
1. Cele SLO (latencja, dostępność, error rate).
2. Spójność i niezawodność (idempotencja, retry, DLQ).
3. Jakość danych (duplikaty, schemat, completeness).
4. Monitoring i alerty (lag, błędy, payload anomalies).
5. Koszt i optymalizacja (egress, retry storm, batch vs streaming).
...
- [ ] SLO/alerty ustawione.
- [ ] Idempotencja/retry/DLQ opisane.
- [ ] Jakość danych monitorowana.
- [ ] Koszt integracji monitorowany.
