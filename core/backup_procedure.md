...
## Cel dokumentu
Opisać wykonanie backupu (co, kiedy, jak), aby dane były spójne, szyfrowane i odzyskiwalne zgodnie z RPO/RTO.

## Zakres i granice
Obejmuje: źródła danych, typ backupu (full/inc/diff/snapshot), okna czasowe, kroki techniczne, weryfikację, logowanie i eskalacje. Nie obejmuje: definicji strategii (jest w Backup and Recovery Strategy) ani testów odtwarzania (osobne procedury).
...
- Prerequisites (dostępy, klucze, wolne miejsce)
- Zakres danych i typ backupu
- Kroki backupu (komendy/narzędzia, kolejność)
- Walidacja (checksum, log, alerty)
- Retencja i lokalizacja (on/off-site)
- Eskalacja i log działań
