---
title: Bezpieczeństwo systemu POS
...
Zabezpieczyć system POS (kasy, terminale) przed fraudem, malware i wyciekami danych kart, zapewniając zgodność z PCI.


## Zakres i granice

- Obejmuje: hardening POS, segmentację sieci, aktualizacje i whitelisting, szyfrowanie danych kart (POI-PAD), integrację z PSP, logi/audyt, fizyczne zabezpieczenia terminali, monitoring integralności.
- Nie obejmuje: polityk cenowych ani procesów sklepu (merchandising).
...
1. Architektura POS i zależności (terminal, serwer, PSP).
2. Hardening i aktualizacje (whitelist, blokada USB, AV/EDR jeśli możliwe).
3. Sieć i izolacja (POS VLAN, firewall, VPN).
4. Dane płatnicze i PCI (POI-PAD, szyfrowanie end-to-end, tokenizacja).
5. Monitoring i audyt (logi, tamper, alerty).
6. Procedury incydentów i serwisu (kompromitacja terminala, swap).
...
- [ ] POS odseparowany w sieci, firewall i VPN.
- [ ] Hardening/aktualizacje/whitelist włączone.
- [ ] Szyfrowanie end-to-end i tokenizacja.
- [ ] Monitoring tamper/logów i procedura incydentów.
- [ ] PCI wymagania spełnione.
