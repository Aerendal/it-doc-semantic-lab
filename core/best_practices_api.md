...
Zebrać dobre praktyki projektowania i utrzymania API (HTTP/REST/GraphQL/gRPC): spójność, bezpieczeństwo, wersjonowanie, wydajność, obserwowalność.


## Zakres i granice

- Obejmuje: konwencje URI/metody/kody, kontrakty i wersjonowanie, paginację/filtry, idempotencję, błędy, bezpieczeństwo (auth/rate limit), wydajność (cache, gzip), schematy (OpenAPI), monitoring/logi/trace.
- Nie obejmuje: szczegółów pojedynczych endpointów (API Reference) ani polityk cenowych API.
...
- Wejścia: wymagania produktowe, typy klientów, standardy org, SLO.
- Wyjścia: guideline do projektów, checklisty review, przykłady payloadów, polityki deprecacji.
...
1. Konwencje (URI, metody, idempotencja, kody).
2. Schematy i walidacja (OpenAPI/JSON Schema, typy).
3. Wersjonowanie i deprecacja.
4. Paginacja/filtry/sortowanie.
5. Bezpieczeństwo (auth, rate limiting, input validation).
6. Wydajność (cache, gzip, bulk, async).
7. Błędy i komunikaty (format, kody).
8. Observability (request ID, logi, trace, metryki).
...
- [ ] OpenAPI/JSON Schema aktualne.
- [ ] Wersjonowanie i polityka deprecacji opisana.
- [ ] Błędy, paginacja, filtry spójne.
- [ ] Auth/rate limit/walidacja wejścia włączone.
- [ ] Observability (ID, trace, metryki) zaimplementowane.


## Definicje robocze

- **Idempotencja** — powtórzenie żądania nie zmienia stanu.
...
- „Konwencje” **defines_structure** „Wersjonowanie i błędy”.
- „Bezpieczeństwo” **constrains** „Limity i pola”.
