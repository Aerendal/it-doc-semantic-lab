...
## Cel dokumentu
Topology Diagram — szablon dokumentu IT.

Zawiera format i wymagania dla rejestru zdarzeń/operacji — co jest logowane, przez kogo, w jakim formacie, przez jaki czas przechowywane i jak chronione przed modyfikacją. Kluczowy element dowodowy dla audytów bezpieczeństwa.
Ten szablon jest zgodny ze standardem **IEEE 1016**.
...
## Wejścia i wyjścia
- **Wejścia** (co musi być dostępne przed wypełnieniem): Polityki bezpieczeństwa (ISO 27001 A.12.4), wymagania regulacyjne (GDPR, NIS2 art.21), retencja danych, wymagania SIEM.
- **Wyjścia** (co dokument wytwarza jako rezultat): Schemat logu z wymaganymi polami (timestamp, actor, action, resource, outcome), polityka retencji, konfiguracja SIEM/log aggregation.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): SIEM (alerty na anomalie), raporty audytów, śledztwa incydentów, dowody compliance dla regulatora.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Polityka bezpieczeństwa informacji, wymagania regulacyjne, architektura systemu (co generuje zdarzenia), polityki dostępu.
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Network Architecture Design', 'type': 'REQUIRES'}
- Zależność: {'depends_on': 'Network Requirements Specification', 'type': 'INFORMED_BY'}
- Zależność: {'depends_on': 'Bandwidth Requirements', 'type': 'INFORMED_BY'}
