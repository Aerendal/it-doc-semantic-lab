...
## Cel dokumentu
Zapewnić identyfikację, ocenę i wdrożenie zmian regulacyjnych/standardów (np. GDPR, DORA, sektorowe) w organizacji w sposób kontrolowany, terminowy i audytowalny.

## Zakres i granice
Obejmuje: monitoring zmian regulacyjnych, ocenę wpływu, plan wdrożenia (procesy/systemy/dane), RACI, komunikację, dowody zgodności i audyt. Nie obejmuje: szczegółowych zmian kodu/procesu – te powstają w dokumentach implementacyjnych.

## Wejścia i wyjścia
- Wejścia: źródła zmian prawa/standardów, ocena prawna/compliance, rejestr systemów i procesów, właściciele domen, terminy wejścia w życie.
- Wyjścia: decyzje o wdrożeniu, plan działań z terminami i ownerami, status zgodności, dowody/audyt zmian, komunikacja do interesariuszy.
