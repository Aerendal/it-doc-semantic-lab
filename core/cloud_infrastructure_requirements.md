---
title: Cloud Infrastructure Requirements
...
## Cel dokumentu
Krótka definicja: Cloud Infrastructure Requirements opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
1. Zakres i model odpowiedzialności: chmury/projekty, regiony, tenancy; Shared Responsibility z dostawcą.
2. IAM: zasady tożsamości (ludzie/maszyny), MFA, RBAC/ABAC, least privilege, konta uprzywilejowane, rotacja kluczy/sekretów.
3. Sieć i segmentacja: VPC/VNet, subnets, security groups/NSG, SGW/Firewall/WAF, prywatne łączność (VPN/Direct Connect/ExpressRoute), egress control.
4. Dane i szyfrowanie: klasyfikacja, szyfrowanie at-rest/in-transit, KMS/HSM, rotacja kluczy, DLP, backup/restore, retention.
5. Obserwowalność i logowanie: centralne logi, metryki, trace, SIEM/SOAR integracje, audit trail, alerty.
6. Compute/containers/serverless: baseline konfiguracji, obrazy golden, skanowanie, patching, pod sandbox/security context, PSP/OPA/Kyverno policy-as-code.
7. Storage/DB: konfiguracje dostępu, kopie, szyfrowanie, PITR, replikacje, ochrona przed usunięciem/immutable.
8. CI/CD i IaC: kontrole w pipeline (lint/scan/sec), SAST/DAST/dep scan, SBOM, policy-as-code, separacja ról.
9. DR/BCP i wydajność: RTO/RPO, regiony AZ, testy DR, autoscaling, limity/quoty.
10. Compliance i evidence: mapowanie wymagań (ISO27001/SOC2/PCI/NIS2) na kontrolki cloud, evidence collection, recertyfikacje dostępu.

## Wymagane rozwinięcia
- IAM/sieć/dane rozwijają: Cloud Security Baseline/Provider Well-Architected, Network Segmentation, KMS policy.
- Observability rozwija: Logging/Monitoring/Alerting standards, SIEM/SOAR playbooks.
- CI/CD/IaC rozwija: Secure SDLC, Supply Chain Security (SBOM, signing).

## Wymagane streszczenia
- Streszczenie kluczowych wymagań bazowych (IAM/sieć/szyfrowanie/logi/DR) i mapowania na normy (ISO/SOC2/PCI).
...
- DoR:
  - [ ] Zakres chmur/projektów i model Shared Responsibility opisane.
  - [ ] Wstępny baseline (IAM, sieć, szyfrowanie, logi, DR) i wymagania norm (ISO/SOC2/PCI) zebrane.
  - [ ] Właściciele domen (IAM, sieć, dane, CI/CD, DR) przypisani.
- DoD:
  - [ ] Wymagania bazowe opisane dla IAM/sieć/szyfrowanie/logi/DR/CI-CD/containers; mapowanie na normy dodane.
  - [ ] Evidence i recertyfikacje zdefiniowane; policy-as-code i skany w pipeline uwzględnione.
  - [ ] Dokument wersjonowany i udostępniony; plan przeglądu/baseline refresh zapisany.
