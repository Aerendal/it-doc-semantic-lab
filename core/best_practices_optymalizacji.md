---
title: Best practices optymalizacji
...
Zebrać praktyki optymalizacji wydajności/kosztów w aplikacjach i infrastrukturze.


## Zakres i granice

- Obejmuje: profilowanie, quick wins, cache, batching, async, redukcję alokacji, tuning baz/kolejek, koszt chmury (rightsizing, storage tiers), obserwowalność pod optymalizacje.
- Nie obejmuje: pełnych testów perf (osobny plan) ani wyboru technologii (ADR).
...
1. Cel optymalizacji i SLO (co poprawiamy).
2. Profilowanie i obserwowalność (metryki, trace).
3. Optymalizacje kodu (algorytmy, alokacje, IO, concurrency).
4. Cache/batching/async i kolejki.
5. Baza/kolejki (indeksy, pooling, query plans).
6. Koszt w chmurze (rightsizing, storage, egress).
7. Walidacja i regresje (testy porównawcze).
...
- [ ] SLO i cel optymalizacji zdefiniowane.
- [ ] Profilowanie/metryki zebrane przed zmianą.
- [ ] Optymalizacje opisane i zmierzone po zmianie.
- [ ] Regresje sprawdzone (test A/B lub before/after).
