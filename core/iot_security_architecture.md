...
Architektura bezpieczeństwa dla rozwiązań IoT: urządzenia, komunikacja, tożsamość, dane, zarządzanie flotą, monitoring i zgodność.


## Zakres i granice

- Obejmuje: klasy urządzeń, provisioning i tożsamość, komunikację (protokół, szyfrowanie), aktualizacje/OTA, zabezpieczenia firmware, segmentację sieci, bezpieczeństwo danych, monitoring/telemetria, zgodność i ryzyka.
- Poza zakresem: szczegółowe buildy firmware’u i konfiguracje per urządzenie (osobne runbooki/BoM).
...
- Kontekst i klasy urządzeń
- Tożsamość/provisioning i zabezpieczenia sprzętowe/firmware
- Komunikacja i segmentacja (protokół, szyfrowanie, edge/gateway)
- Aktualizacje/OTA i bezpieczeństwo łańcucha dostaw
- Dane: klasyfikacja, szyfrowanie, retencja, prywatność
- Monitoring/telemetria, detection/response
...
- Wypisz klasy urządzeń, provisioning, kanały i kontrolki; sekcje N/A uzasadnij.
- Dodaj quick-links i checklisty DoR/DoD w `reports/checklist_atomic.jsonl`.
- Podlinkuj wyniki testów/pentestów i plan OTA; aktualizuj przy zmianach floty.
...
- Katalog urządzeń i profili (hardware/OS/protokół), mapa danych i przepływów.
- Polityki bezpieczeństwa IoT, wymagania regulacyjne/branżowe, benchmarki.
- Wyniki pentestów/scanów firmware, incydenty i audyty.


## Wyjścia

- Diagramy architektury IoT (device→gateway/cloud→apps).
- Kontrolki i wymagania per warstwa (device, edge, cloud, app, data).
- Rekomendacje i plan działań/OTA.
...
- Tabela: klasa urządzenia → protokół → auth → szyfrowanie → OTA → monitoring.
- Streszczenie ryzyk i rekomendacji.


## Wymagane powiązania

- Polityki IoT, supply chain/firmware security, CSPM/SIEM, rejestr ryzyk.


## Kryteria DoR

- [ ] Katalog urządzeń i przepływy danych dostępne.
- [ ] Polityki/regulacje IoT zebrane.
- [ ] Wyniki testów/pentestów dostępne.


## Kryteria DoD

- [ ] Diagramy i kontrolki opisane.
- [ ] Plan OTA/ryzyka i rekomendacje dodane.
- [ ] Quick-links/checklisty zaktualizowane, metadane bieżące.


## Artefakty do załączenia

- Diagramy device→gateway→cloud.
- Tabela kontrolki per klasa urządzeń.
- Plan OTA i wyniki testów.


## Walidacja / testy

- Peer review architektury; sanity kryptografii/kluczy.
- Testy OTA/rollback; weryfikacja segmentacji sieci.


## Metryki monitorowane

- % urządzeń z aktualnym firmware/kluczami.
- Liczba incydentów IoT i czas reakcji.
- Wyniki testów/pentestów per klasa urządzeń.


## Utrzymanie i aktualizacje

- Przegląd po releasach firmware/OTA lub incydentach; co najmniej kwartalnie.
- Aktualizuj katalog urządzeń i kontrolki przy nowych modelach/protokółach.


## Zakończenie

Po spełnieniu DoD opublikuj dokument, podlinkuj artefakty, odhacz checklisty w `reports/checklist_atomic.jsonl` i zsynchronizuj z planem OTA/floty.
