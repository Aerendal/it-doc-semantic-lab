...
Zapewnić spójną i bezpieczną integrację API dla aplikacji mobilnych, z naciskiem na wydajność, wersjonowanie i odporność.
...
1. Zakres API: krytyczne ścieżki (login, płatności, katalog, koszyk itp.), właściciele.
2. Wersjonowanie i kompatybilność: polityka wersji, feature flags, deprecacje.
3. Wydajność i sieć: cache/ETag, kompresja, retry/backoff, ograniczanie payloadów.
4. Bezpieczeństwo: OAuth/OIDC, TLS pinning, rate limit, detekcja nadużyć, ochrona PII.
5. Observability: tracing mobile→backend, metryki (latency, błędy), logi klienta.
6. Testy: kontraktowe, urządzenia/sieci (3G/roaming), chaos (utrata pakietów), canary release.
...
- [ ] Polityka wersji i deprecacji API opisana i zakomunikowana.
- [ ] Krytyczne ścieżki mają cache/ETag i retry/backoff dobrane do mobilnych sieci.
- [ ] Zabezpieczenia (OAuth, pinning, rate-limit, PII) wdrożone i przetestowane.
- [ ] Obserwowalność end-to-end i testy na realnych warunkach sieciowych wykonane.
