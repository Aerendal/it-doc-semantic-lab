...
Ustalić cele wydajnościowe systemu/aplikacji (latencja, throughput, zasoby), aby kierować architekturą, testami i monitorowaniem.


## Zakres i granice

- Obejmuje: SLO/SLI wydajnościowe, targety (p95/p99 latencja, TPS/QPS, error rate), budżet wydajności, scenariusze krytyczne, monitorowanie i testy.
- Nie obejmuje: szczegółowych planów optymalizacji (osobny dokument).
...
1. Krytyczne ścieżki i metryki (p95/p99, throughput, error).
2. Targety/SLO i budżet wydajności (na komponenty).
3. Metodologia pomiaru (RUM/APM, synthetic, sample).
4. Testy i weryfikacja (load/stress/spike, okresowość).
5. Monitoring/alerty i przeglądy.
...
- [ ] SLO/targety zdefiniowane na kluczowe ścieżki.
- [ ] Metody pomiaru opisane.
- [ ] Plan testów perf i częstotliwość.
