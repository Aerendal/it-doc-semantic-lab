...
Zaplanować wdrożenie Customer Data Platform: zasilanie danymi, model zgód, segmentacja i aktywacja.
...
1. Zakres danych: źródła (web/app/CRM/POS), identyfikacja klienta, model zgód.
2. Architektura: ingest, identity resolution, profile, katalog zdarzeń, storage.
3. Segmentacja i reguły biznesowe: definicje, właściciele, wersjonowanie.
4. Aktywacja: kanały (email/push/ads), częstotliwość, limity, testy.
5. Bezpieczeństwo i prywatność: RODO, retention, masking, audyt dostępu.
6. Metryki: pokrycie profili, match rate, skuteczność kampanii, latency.
...
- [ ] Źródła danych i model zgód zmapowane.
- [ ] Identity resolution i schemat profilu opisane.
- [ ] Segmenty mają właścicieli, wersjonowanie i testy.
- [ ] Polityki prywatności/retencji i audyt dostępu wdrożone.
