...
## Cel dokumentu
Zdefiniować ValueSet i terminologię stosowaną w zasobach HL7 FHIR — jako formalne definicje dopuszczalnych wartości kodowanych w zasobach klinicznych (CapabilityStatement.kind, Observation.status itp.). ValueSet Definitions zapewniają interoperacyjność semantyczną między systemami zdrowotnymi: standaryzują kody diagnoz (ICD-10, SNOMED CT), leków (RxNorm), wyników laboratoryjnych (LOINC) i procedur.
