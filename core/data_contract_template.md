...
Szablon kontraktu danych między producentem a konsumentem: schemat, SLA, wersjonowanie, odpowiedzialności.
...
- [ ] Schemat + semantyka + przykłady.
- [ ] Wersjonowanie i kompatybilność opisane.
- [ ] SLA DQ (freshness, completeness) zdefiniowane.
- [ ] Bezpieczeństwo/PII i access opisane.
...
1. Strony i kontakt (owner producer/consumer).
2. Zakres i schemat (pola, typy, semantyka, przykłady).
3. Wersjonowanie i kompatybilność (policy, breaking/non-breaking).
4. Jakość i SLA (freshness, completeness, timeliness, avail).
5. Bezpieczeństwo/PII (klasyfikacja, maskowanie, access).
6. Zmiany i deprecacja (proces, komunikacja).
7. Monitoring i alerty (DQ checks, SLO).
