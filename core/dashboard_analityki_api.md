...
Zaprojektować dashboard do obserwacji użycia API: wolumen, wydajność, błędy, klienci, wersje.
...
1. Metryki główne: requests, error rate, latency (p50/p95/p99), payload size.
2. Segmentacja: endpoint, metoda, klient/klucz, wersja, region.
3. Health/quality: throttling/rate limit hits, auth failures, timeouts.
4. Koszt: egress, top noisy clients, heavy queries.
5. Alerty: error spikes, latency regressions, abnormal traffic.
6. Drill‑down: sample requests, logs/trace correlation.
...
- [ ] Metryki i segmenty skonfigurowane.
- [ ] Alerty na błędy/latencję ustawione.
- [ ] Rate limits/throttling monitorowane.
- [ ] Koszt/egress i top clients widoczne.
