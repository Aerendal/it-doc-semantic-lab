...
## Cel dokumentu
Opisać, jak system spełnia wymagania 21 CFR Part 11 dla elektronicznych zapisów (ER) i podpisów (ES): walidacja systemu, tożsamość i uprawnienia, podpisy, integralność danych, audit trail, zmiany, backup/archiwizacja, dostawcy, szkolenia oraz dowody dla audytu FDA.

## Zakres i granice
- Obejmuje: systemy/komponenty z ER/ES, walidację CSV (IQ/OQ/PQ), kontrolę dostępu, podpisy elektroniczne, audit trail, integralność/backup/archiwizację, change control, dostawców, SOP i szkolenia, przeglądy okresowe.
- Poza zakresem: szczegółowe procedury pen-test/DR (linki) oraz implementacja kryptografii (odnosimy do KMS/PKI dokumentów).

## Wejścia i wyjścia
- Wejścia: wymagania Part 11, polityki ER/ES, specyfikacje wymagań, plan walidacji, protokoły/testy IQ/OQ/PQ, IAM/KMS/PKI konfiguracje, polityki backup/retencji, SOP, rejestr szkoleń, oceny dostawców.
- Wyjścia: plan i dowody zgodności Part 11, raport walidacji, matryca wymagania↔testy, opis podpisów i audit trail, rejestr zmian i rewalidacji, raport z przeglądu okresowego, CAPA.
...
- Zakres: systemy, typy ER/ES, klasy danych (GxP), środowiska.
- Walidacja (CSV): plan, IQ/OQ/PQ, traceability matryca, raport walidacji, deviation/CAPA.
- Kontrola dostępu: unikalne ID, MFA, role/SoD, blokady, revocation, logowania.
- Podpisy elektroniczne: składniki (ID+hasło/biometria), powiązanie z zapisem, certyfikaty, timestamp, integrity check.
- Integralność danych: audit trail (kto/co/kiedy/dlaczego), nieusuwalność, wersjonowanie, backup/retencja/archiwizacja, synchronizacja czasu.
- Change Control i rewalidacja: kryteria zmian wymagających rewalidacji, dokumentacja konfiguracji.
- Dostawcy: kwalifikacja, umowy (CSV/SLA), audyty dostawców, ocena ER/ES u vendorów.
- SOP i szkolenia: lista SOP, rejestr szkoleń, egzaminy/świadectwa, cykl odświeżania.
- Przeglądy okresowe: harmonogram, przegląd audit trail, review podpisów/hasła/MFA, raport zgodności, CAPA.

## Wymagane rozwinięcia
- Walidacja: Validation Plan/Report, Requirements/Specs, Test Protocols IQ/OQ/PQ, Traceability Matrix.
- Audit trail/integralność: Logging/Monitoring Design, Backup/DR, Data Retention, Time Sync/NTP.
- Podpisy elektroniczne: IAM, PKI/KMS, Key/Cert Management.
- Dostawcy: Vendor Qualification/TPRM, umowy CSV/SLA, audyty.
...
- Streszczenie statusu walidacji (IQ/OQ/PQ, deviation/CAPA).
- Streszczenie kontroli podpisów/audit trail i otwartych luk.
- Streszczenie przeglądu okresowego i planu rewalidacji.
...
- DoR:
  - [ ] Zdefiniowany zakres systemów ER/ES i jurysdykcji; dostępne wymagania Part 11.
  - [ ] Plan walidacji i protokoły IQ/OQ/PQ przygotowane; środowiska testowe gotowe.
  - [ ] Role (CSV lead, QA, Security, Compliance) i narzędzia (IAM/PKI/logi) uzgodnione.
- DoD:
  - [ ] Walidacja (IQ/OQ/PQ) wykonana; traceability i raport walidacji uzupełnione.
  - [ ] Kontrola dostępu/podpisów/audit trail opisana z dowodami; synchronizacja czasu potwierdzona.
  - [ ] Change Control i rewalidacja zdefiniowane; przegląd okresowy zaplanowany.
  - [ ] Dostawcy/umowy CSV ocenione; SOP/szkolenia i rejestry uzupełnione.
  - [ ] Dokument wersjonowany; linki do dowodów działają; sekcje N/A uzasadnione.
