...
Standaryzować tworzenie aplikacji React Native.
...
1. Architektura: state management (Redux/RTK/React Query), modularność, native modules.
2. UI: design system, dostępność, responsywność, theming.
3. Dane: networking (fetch/axios), caching, offline, synchronizacja.
4. Build/CI: Metro config, signing Android/iOS, OTA updates (CodePush/OTA).
5. Wydajność: bridge usage, re-rendering, memory leaks, animacje.
6. Bezpieczeństwo: storage, obfuscation, perms, komunikacja z native.
...
- [ ] Wybrano wzorzec state management i strukturę modułów.
- [ ] Testy e2e (Detox/Appium) i unit są w pipeline.
- [ ] OTA aktualizacje i signing skonfigurowane; monitoring crash/ANR.
- [ ] Wydajność i bezpieczeństwo są profilowane i w budżecie.
