...
Określić zasady bezpieczeństwa dla systemów Industrial Control Systems (ICS/SCADA): segmentacja, kontrola dostępu, monitoring, patching i response.


## Zakres i granice

- Obejmuje: segmentację sieci OT/IT, dostęp zdalny, hardening urządzeń/PLC/HMI, zarządzanie łatkami, monitoring anomalii, backup/restore, incident response w OT.
- Nie obejmuje: bezpieczeństwa fizycznego i procedur BHP.
...
1. Segmentacja i architektura (zones/conduits, firewalle, DMZ, jump hosts).
2. Dostęp i uwierzytelnianie (MFA gdzie możliwe, konta serwisowe, least privilege).
3. Hardening i patching (aktualizacje, listy kontrolne vendorów).
4. Monitoring i detekcja (IDS/IPS OT, logi, anomalie).
5. Backup/restore i testy.
6. Incident response w OT (playbook, eskalacja, izolacja).
...
- [ ] Segmentacja OT/IT wdrożona.
- [ ] Dostęp zdalny kontrolowany (jump host/MFA).
- [ ] Hardening i patching zaplanowane.
- [ ] Monitoring anomalii działa.
- [ ] Backup/restore i IR dla OT przetestowane.
