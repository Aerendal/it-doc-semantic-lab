...
## Cel dokumentu
Przeprowadzić audyt zgodności z ADA/WCAG/EN 301 549 dla wszystkich kanałów (web/mobile/PDF/wideo/kiosk), zidentyfikować defekty dostępności, zaplanować remediację i przygotować dowody (VPAT/ACR), by zredukować ryzyko prawne i odrzuceń.

## Zakres i granice
- Obejmuje: audyt manualny i automatyczny wg WCAG 2.1/2.2 (A/AA), EN 301 549, ADA; kanały web/mobile/PDF/wideo/kiosk; wzorce UI i komponenty; defekty + priorytety; plan napraw; VPAT/ACR; proces utrzymania regresji.
- Poza zakresem: pełne wytyczne projektowe (odsyłamy do Accessibility Compliance/Design System) oraz implementacje komponentów (linki).

## Wejścia i wyjścia
- Wejścia: zakres ekranów/flow, buildy testowe, standardy (WCAG/EN/ADA), lista komponentów/design system, narzędzia (NVDA/JAWS/VoiceOver/TalkBack, axe/pa11y), wymagania prawne lokalne, polityka prywatności/terms.
- Wyjścia: raport audytu (defekty + priorytety), VPAT/ACR, plan remediacji z ownerami i ETA, lista ryzyk prawnych, zalecenia dla design/dev/QA, metryki (% zgodności, top defekty).
...
- Zakres audytu: standardy (WCAG 2.1/2.2 A/AA, EN 301 549, ADA), kanały, urządzenia, języki.
- Metody i narzędzia: testy manualne (klawiatura, screen reader), automaty (axe/pa11y), kolor/kontrast, focus, ARIA, multimedia/captions, PDF tagging.
- Tabela defektów: kryterium, kanał/ekran, severity (blocker/critical/major/minor), opis, repro, dowód (screenshot/video), właściciel, ETA, status, link do ticketu.
- VPAT/ACR: sekcje, status (supports/partially/does not), dowody.
- Plan remediacji i retesty: kolejność napraw, terminy, odpowiedzialni, kryteria akceptacji, retest/regresja.
- Utrzymanie: cadence audytów, PR review checki, checklisty dev/QA, szkolenia.

## Wymagane rozwinięcia
- WCAG/EN 301 549 mapping → Accessibility Compliance/Design System.
- Testy i narzędzia → QA Test Plan (sekcja A11y), Tooling Guide.
- VPAT/ACR → wymagania klienta/regulatora.

## Wymagane streszczenia
- Streszczenie statusu A11y: % kryteriów zgodnych, top defekty krytyczne, ETA napraw, ryzyka prawne.
...
- DoR:
  - [ ] Zakres standardów/kanałów/urządzeń ustalony; buildy testowe dostępne.
  - [ ] Role i narzędzia (screen readers/axe/pa11y) uzgodnione; lista ekranów/komponentów gotowa.
- DoD:
  - [ ] Defekty opisane z severity/dowodami/ownerami/ETA; VPAT/ACR uzupełniony.
  - [ ] Plan remediacji i retestów zapisany; ryzyka prawne (jeśli) wskazane.
  - [ ] Plan utrzymania/regresji A11y dodany; dokument wersjonowany i udostępniony; sekcje N/A uzasadnione.
