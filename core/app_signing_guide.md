...
Opisać proces podpisywania aplikacji (mobile/desktop) kluczami certyfikacyjnymi, aby zapewnić autentyczność, integralność i zgodność z wymaganiami platform.


## Zakres i granice

- Obejmuje: rodzaje kluczy/certyfikatów (release/debug), generowanie/przechowywanie, rotację, CI/CD signing, platformowe wymagania (Android/iOS/macOS/Windows), bezpieczeństwo kluczy.
- Nie obejmuje: dystrybucji aplikacji (store deployment) ani polityk MDM (osobne dokumenty).
...
- Wejścia: wymagania platform, polityka bezpieczeństwa kluczy, konta deweloperskie, pipeline CI/CD.
- Wyjścia: procedura signing, lokalizacja i ochrona kluczy, konfiguracja CI/CD, runbook rotacji i odwołania kluczy.
...
1. Typy kluczy/certyfikatów i kiedy używać (debug/release, platformy).
2. Generowanie i przechowywanie (HSM/keystore/Keychain, secret store).
3. Signing w CI/CD (konfiguracja, secrety, podpisywanie artefaktów).
4. Rotacja i odwołanie kluczy (kiedy, jak, komunikacja).
5. Bezpieczeństwo i audyt (dostępy, logi, backup).
...
- [ ] Klucze release/debug zidentyfikowane, bezpiecznie przechowywane.
- [ ] CI/CD podpisuje automatycznie; secrety nie wypływają.
- [ ] Rotacja i odwołanie klucza opisane.
- [ ] Dostępy i audyt do kluczy ograniczone.


## Definicje robocze

- **Signing** — kryptograficzne potwierdzenie, że aplikacja pochodzi od wydawcy i nie została zmodyfikowana.
- **Keystore/Keychain/HSM** — bezpieczne miejsce przechowywania kluczy.
...
- „Klucze i certyfikaty” **defines_structure** „Proces signing”.
- „Rotacja/odwołanie” **constrains** „Utrzymanie w CI/CD”.
