...
Zaprojektować monitoring FPS/frametime w aplikacjach (gry/immersive/3D), aby wykrywać degradacje wydajności i utrzymać doświadczenie użytkownika w ramach SLO.


## Zakres i granice

- Obejmuje: metryki FPS/frametime (p50/p95/p99), stutter, frame drops, GPU/CPU usage, VRAM, termiki, per sceny/biomy/urządzenia, telemetria prod, testy syntetyczne, progi alertów, dashboardy, korelacja z buildami/assetami, privacy i koszty.
- Poza zakresem: pełna optymalizacja grafiki (osobne dokumenty), anti-cheat.
...
SLO → progi alertów; device matrix → sampling; zmiany assetów → regresje; telemetry → dashboardy.


## Wymagane rozwinięcia

- Event schema → pola i jednostki.
- Progi → per device/scene.


## Wymagane streszczenia

- Tabela device→scene→FPS p95/p99 + alerty.


## Guidance

Cel: szybkie wykrywanie regresji FPS. DoR: SLO, device matrix, telemetry stack, sceny. DoD: instrumentacja/dash/alerty/testy/progi opisane; sekcje N/A uzasadnione; metadane aktualne.


## Checklisty jakości (DoR/DoD skrót)

- DoR: [ ] SLO; [ ] Device matrix i sceny; [ ] Telemetry stack; [ ] Privacy.
- DoD: [ ] Instrumentacja/testy/dash/alerty gotowe; [ ] Raporty i progi; [ ] Sekcje N/A uzasadnione; metadane aktualne.
