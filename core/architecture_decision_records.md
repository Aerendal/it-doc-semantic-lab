...
Utrwalić ważne decyzje architektoniczne wraz z kontekstem, wariantami, uzasadnieniem i konsekwencjami, żeby zespół wiedział, dlaczego podjęto taki wybór.


## Zakres i granice

- Obejmuje: decyzje na poziomie architektury (technologie, wzorce, integracje, dane, bezpieczeństwo, operacje), ich datę, status, ownera, skutki.
- Nie obejmuje: drobnych decyzji implementacyjnych (issue/PR), planów projektowych.
...
- Wejścia: kontekst problemu, wymagania, ograniczenia, warianty.
- Wyjścia: zapis ADR (Context → Decision → Consequences), linki do PoC/analiz, status (proposed/accepted/deprecated/superseded).
...
1. Tytuł i ID ADR.
2. Status (proposed/accepted/deprecated/superseded), data, owner.
3. Context (problem, wymagania, ograniczenia).
4. Decision (wybór, zakres).
5. Alternatives rozważone (plusy/minusy).
6. Consequences (pozytywne/negatywne, wpływ na bezpieczeństwo, operacje, koszty).
7. Linki (PoC, diagramy, PRD, RFC).
...
- [ ] Jasny kontekst i wymagania.
- [ ] Decyzja i alternatywy opisane.
- [ ] Konsekwencje (plus/minus) zidentyfikowane.
- [ ] Status i owner ustawione.
- [ ] Linki do dowodów/PoC dołączone.


## Definicje robocze

- **ADR** — pojedynczy zapis decyzji architektonicznej.
- **Superseded** — decyzja zastąpiona nowszą.
...
- „Kontekst” **defines_structure** „Decyzję i konsekwencje”.
- „Status” **constrains** „Następne działania (np. supersede)”.
