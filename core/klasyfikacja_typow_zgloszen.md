---
title: Klasyfikacja typów zgłoszeń
...
Ustalić spójny podział zgłoszeń (incydenty, problemy, zadania, zmiany) dla supportu i developmentu.



## Zakres i granice
Obejmuje: cel zadania, pre‑requisites, kroki z komendami, oczekiwane wyniki, kryteria sukcesu, ścieżki awaryjne, eskalacje, kontakt, artefakty do logowania. Nie obejmuje: decyzji produktowych ani zmian architektury.
...
1. Typy zgłoszeń i definicje: incydent, problem, change, request, task, improvement.
2. Kryteria klasyfikacji: wpływ/pilność, SLA, priorytety, właściciele.
3. Ścieżki obsługi: routing do zespołów, eskalacje, komunikacja.
4. Dane wymagane w zgłoszeniu: minimalne pola, załączniki, logi, reprodukcja.
5. Workflow i statusy: nowy → w toku → oczekuje → rozwiązany → zamknięty; recykling.
6. Raportowanie: metryki (MTTR, backlog), kategorie top, zgodność SLA.
...
- [ ] Typy zgłoszeń i kryteria wpływ/pilność opisane.
- [ ] Routing/escalation i wymagane dane w zgłoszeniu zdefiniowane.
- [ ] Statusy i SLA przypisane; metryki raportowane.
- [ ] Szablony zgłoszeń dostępne dla użytkowników/agentów.
