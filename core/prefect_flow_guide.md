...
Przygotować wytyczne tworzenia, wersjonowania i utrzymania flow w Prefect (2.x).
...
1. Struktura repo: katalogi na flows/blocks, konwencje nazw, tagi.
2. Bloki i konfiguracja: storage, infrastructure, secrets, parametrization, profiles.
3. Dobre praktyki flow: retries, timeouts, concurrency, caching, logging, mapping/subflows.
4. Deployment: build/deploy, schedules, worker setup, automations, promos/staging→prod.
5. Observability: UI, metrics, alerts, trace/log retention, failures playbook.
6. Bezpieczeństwo: secrets management, least privilege dla workers, upgrades, backups.
...
- [ ] Repo/flow naming i bloki (storage/infra/secrets) zgodne z wytycznymi.
- [ ] Retries/timeouts/concurrency i logging ustawione w flow.
- [ ] Deploy/schedule i automations działają; promo do prod ma procedurę.
- [ ] Monitoring/alerts i playbook awarii Prefecta gotowe; sekrety chronione.
