...
Opisać standardy definiowania i utrzymania Assetów w Dagster: zależności, materialization, observability i testy.
...
1. Naming/metadata Assetów (opis, group, tags, owners).
2. Zależności i partitioning (source assets, partitions, IO managers).
3. Materialization i schedulers/sensors (freshness policies).
4. Observability (asset checks, events, alerty, dashboards).
5. Testy i lokalne uruchamianie (unit, dagit playground).
6. Deploy i wersjonowanie kodu pipeline.
...
- [ ] Assets mają opis/tags/owner.
- [ ] Zależności/partitions poprawnie zdefiniowane.
- [ ] Freshness/materialization polityki ustawione.
- [ ] Asset checks/alerty działają.
- [ ] Testy i deploy pipeline opisane.
