...
Business Impact Analysis — szablon dokumentu IT.

Określa cele odtworzeniowe (RTO, RPO) i procedury przywracania usług po katastrofie lub poważnej awarii. Zawiera: krytyczne systemy i priorytety odtworzenia, kolejność działań, alternatywne lokalizacje/środowiska, harmonogram testów BCP/DR.
...
- **Wejścia** (co musi być dostępne przed wypełnieniem): Analiza wpływu na biznes (BIA), inwentarz systemów (CMDB), umowy SLA z dostawcami, wyniki ostatnich testów DR, topologia sieci i infrastruktury.
- **Wyjścia** (co dokument wytwarza jako rezultat): Plan testów DR, harmonogram ćwiczeń, raporty z testów, zaktualizowane procedury runbooka, metryki RTO/RPO.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): Plany testów DR, raporty z ćwiczeń, umowy z dostawcami cloud/hostingiem, polityki retencji backupów, plany komunikacji kryzysowej.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Analiza BIA, rejestr zasobów (CMDB), polityka backupów, plany zarządzania incydentami.
