...
## Cel dokumentu
Opisać krok po kroku procedurę obsługi zmian (standard/normal/awaryjna): od zgłoszenia, przez ocenę wpływu i zatwierdzenie, po wdrożenie, walidację i zamknięcie. Zapewnia jednolity, audytowalny przebieg zmian.

## Zakres i granice
Obejmuje: klasy zmian, kryteria kwalifikacji, ścieżki zatwierdzeń, wymagane informacje w zgłoszeniu, oceny wpływu (biznes/tech/sec), wymagania testów/regresji, plan komunikacji i rollback, zamknięcie i audyt. Nie obejmuje: szczegółów technicznych deploy (Runbook).

## Wejścia i wyjścia
- Wejścia: Change Management Plan, kalendarz releasów/okien, lista zależności i systemów, polityki bezpieczeństwa/compliance, backlog zmian.
- Wyjścia: wykonana procedura zmiany z logiem działań, decyzje CAB/ownerów, wynik testów/regresji, status Go/No-Go, zapis audytowy.
...
- Konsumuje: Change Management Plan, Release Plan, Incident/Problem records, Service Catalog.
- Dostarcza do: Deployment/Runbook, komunikacji statusowej, raportów audytowych.
...
- Klasy zmian i kryteria kwalifikacji (standard/normal/awaryjna)
- Wymagane pola zgłoszenia i ścieżki zatwierdzeń (CAB, właściciel usługi, bezpieczeństwo)
- Ocena wpływu (biznes/klient/technika/bezpieczeństwo)
- Wymagania testów/regresji i plan rollback/backout
- Okna wdrożeniowe, blackouty i zależności
- Komunikacja (kto/kiedy/jakie kanały)
- Wykonanie, walidacja, zamknięcie i zapis audytowy
