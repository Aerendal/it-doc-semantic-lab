...
Typowe problemy z wideo (stream/VOD) i ich rozwiązania.
...
- [ ] ABR ladder i CDN zoptymalizowane.
- [ ] Per-title/codec fallback zapewniony.
- [ ] Sync A/V testowany.
- [ ] Startup time monitorowany (First Frame).
- [ ] DRM/compatibility testy na gł. urządzeniach.
...
- **Buffering/lag**: zbyt mały bitrate adaptacyjny → ABR ladder dopasowana, CDN, prefetch, większy cache playera.
- **Jakość niska**: brak transkodów → multi-bitrate/HLS/DASH, poprawny ladder, per-title encoding.
- **Audio/video desync**: timestamp drift → resync w transkodzie, align timestamps, player sync.
- **Startup time długi**: za duży manifest/segment → krótsze initial segmenty, szybszy DNS/CDN, lazy subtitle load.
- **Device compatibility**: kodeki → H.264 fallback, DRM widevine/fairplay, feature detect.
- **Stall przy zmianie jakości**: duże skoki bitrate → smooth ABR, capped jumps, hybrid buffer.
