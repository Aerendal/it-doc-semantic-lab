---
title: Cele inteligencji on-device
...
Zdefiniować cele dla modeli/algorytmów działających na urządzeniu (on-device AI): wydajność, prywatność, niezawodność i doświadczenie użytkownika.


## Zakres i granice

- Obejmuje: metryki jakości modelu na urządzeniu, latencję i zużycie zasobów (CPU/GPU/NPU/bateria), prywatność danych lokalnych, update modeli OTA, fallback offline, telemetry.
- Nie obejmuje: trenowania w chmurze ani architektury backend.
...
1. Metryki jakości (accuracy/latency/power/footprint) i targety.
2. Zużycie zasobów i limity (CPU/NPU, RAM, storage, bateria).
3. Prywatność i dane lokalne (minimalizacja, szyfrowanie).
4. Aktualizacje modeli OTA i rollback.
5. Telemetria i monitoring na urządzeniu (zgody, sampling).
6. UX/fallback (offline, degradacja gracji).
...
- [ ] Targety jakości/latencji/power ustalone.
- [ ] Limity zasobów zdefiniowane.
- [ ] Prywatność i szyfrowanie danych lokalnych.
- [ ] OTA/rollback opisane.
- [ ] Telemetria z zgodami i samplingiem.
