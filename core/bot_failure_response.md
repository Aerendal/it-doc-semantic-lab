...
Opisać, jak reagować na awarie bota (chat/voice/automation): wykrywanie, fallback, komunikacja i naprawa, by minimalizować frustrację użytkowników.


## Zakres i granice

- Obejmuje: detekcję błędów (timeouty, 5xx, intent miss), fallback (człowiek/live chat, prosty formularz), komunikaty dla użytkownika, logowanie i eskalacje, metryki.
- Nie obejmuje: trenowania NLU (osobny dokument) ani strategii treści konwersacyjnych.
...
1. Scenariusze awarii i detekcja (sygnały, progi).
2. Fallback/tryb awaryjny (co pokazujemy, przekierowanie do agenta).
3. Komunikaty i UX awarii (jasne, bez winy użytkownika).
4. Logi/metryki i eskalacje (SRE/produkt).
5. Post-incident (analiza przyczyn, poprawki).
...
- [ ] Scenariusze awarii z progami.
- [ ] Fallback działający (agent/formularz).
- [ ] Komunikaty user-friendly.
- [ ] Logi/metryki i eskalacje ustawione.
- [ ] Post-incident RCA i poprawki.
