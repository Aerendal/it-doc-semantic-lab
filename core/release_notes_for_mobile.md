---
title: Release Notes for Mobile
...
## Cel dokumentu
Release Notes for Mobile — szablon dokumentu IT.

Opisuje plan wdrożenia nowej wersji systemu — kroki, zależności, rollback plan, kryteria go/no-go, komunikację do interesariuszy i weryfikację post-deploy.
...
## Wejścia i wyjścia
- **Wejścia** (co musi być dostępne przed wypełnieniem): Specyfikacja release (changelog, scope zmian), wyniki testów QA, konfiguracja środowisk, okno zmian (change window), listy powiadomień.
- **Wyjścia** (co dokument wytwarza jako rezultat): Zatwierdzone okno zmian (CAB), kroki wdrożeniowe z odpowiedzialnościami, plan rollback z kryteriami aktywacji, smoke tests, raport z wdrożenia.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): Rejestr zmian, metryki monitoringu post-deploy, plan komunikacji z użytkownikami, dokumentacja operacyjna.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Raport z testów QA, polityka zarządzania zmianami (ITIL), harmonogram releaseów, konfiguracja CI/CD.
