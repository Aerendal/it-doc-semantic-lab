...
## Cel dokumentu
Krótka definicja: Cloud Best Practices opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
1. Architektura i model odpowiedzialności: chmury/projekty/regiony, Shared Responsibility, landing zone.
2. IAM: RBAC/ABAC, MFA, konta uprzywilejowane, tożsamość maszynowa, rotacja sekretów, SSO.
3. Sieć: segmentacja (VPC/VNet), SG/NSG/WAF, egress control, prywatna łączność, DNS, TLS everywhere.
4. Dane: klasyfikacja, szyfrowanie at-rest/in-transit, KMS/HSM, klucze/rotacja, backup/retencja, DLP.
5. Compute/Containers/Serverless: hardened images, patching, skanowanie, minimalne uprawnienia, isolation/policy-as-code.
6. Observability: logi/metryki/trace, centralny eksport, SIEM/SOAR integracje, alerting, SLO/error budget.
7. CI/CD i IaC: kontrola zmian, SAST/DAST/dep scan, SBOM/podpisy, policy-as-code, separacja ról, approvals.
8. Resilience/DR/BCP: RTO/RPO, multi-AZ/region, testy DR, chaos/soak, runbooki, backup/restore testy.
9. Koszty i wydajność: tagging/FinOps, autoscaling, right-sizing, budżety/alerty kosztowe.
10. Compliance i dowody: mapowanie norm (ISO/SOC2/PCI/NIS2), recertyfikacje, evidence collection, audyty.

## Wymagane rozwinięcia
- IAM/sieć/dane rozwijają: Cloud Infrastructure Requirements, Cloud Compliance Roadmap, Provider baseline/CIS.
- Observability/DR rozwija: Logging/Monitoring standards, SRE/DRP/BCP runbooks.
- CI/CD/Supply chain rozwija: Secure SDLC, Supply Chain Security (SBOM/signing), Change Management.

## Wymagane streszczenia
- Streszczenie top 10 praktyk bazowych (IAM/sieć/szyfrowanie/logi/DR/CI-CD) i mapowania na normy.
...
- DoR:
  ...
  - [ ] Baseline wymagań (IAM/sieć/szyfrowanie/logi/DR/CI-CD) zebrany; role ownerów przypisane.
  - [ ] Standardy/normy do mapowania zidentyfikowane (ISO/SOC2/PCI/NIS2).
- DoD:
  - [ ] Praktyki bazowe opisane dla IAM, sieci, danych, compute, obserwowalności, CI/CD, DR, FinOps.
  - [ ] Policy-as-code/skany/recertyfikacje i evidence ujęte; mapowanie na normy dodane.
  - [ ] Dokument wersjonowany i udostępniony; plan przeglądu okresowego (refresh baseline) zapisany.
