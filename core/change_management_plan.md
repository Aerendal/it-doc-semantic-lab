...
## Cel dokumentu
Określić proces zarządzania zmianą (technologiczną/procesową) tak, by była kontrolowana, bezpieczna i komunikowana. Zapewnia jasne kroki od zgłoszenia zmiany po wdrożenie i retrospektywę.

## Zakres i granice
Obejmuje: typy zmian (standard/normal/awaryjna), ścieżki zatwierdzeń, oceny wpływu (biznes/technika/bezpieczeństwo), okna wdrożeń, komunikację, rollback, audyt. Nie obejmuje: szczegółów technicznego deploymentu (Runbook) ani zarządzania wymaganiami.

## Wejścia i wyjścia
- Wejścia: backlog zmian, oceny ryzyka, Dependency list, polityki bezpieczeństwa i SLA, kalendarz releasów.
- Wyjścia: zaakceptowany proces CM, formularz/zbiór pól w zgłoszeniu zmiany, RACI dla zatwierdzeń, harmonogram okien, zasady rollback/komunikacji.
...
- Konsumuje: Architecture/Security governance, Release Plan, Incident/Problem mgmt, Service Catalog.
- Dostarcza do: Deployment Plan, Runbooków, komunikacji statusowej, audytów compliance.

- Typy zmian ↔ ścieżki zatwierdzeń i SLA.
- Ocena wpływu ↔ plan testów/regresji i komunikacja.
- Rollback/Backout ↔ Runbook i Incident mgmt.
...
- Zakres stosowania i definicje typów zmian
- Role i RACI (wnioskodawca, CAB, właściciel usługi, bezpieczeństwo)
- Proces end-to-end: zgłoszenie → ocena wpływu → aprobata → plan wdrożenia → komunikacja → wykonanie → walidacja → zamknięcie
- Kryteria oceny wpływu (biznes, klient, technika, bezpieczeństwo)
- Okna wdrożeniowe i blackouty
- Wymagania testów/regresji i plan rollback/backout
- Komunikacja (kanały, template komunikatu, kto/kiedy)
- Rejestr zmian i audyt / raportowanie
...
## Szybkie powiązania
- Zależność: {'depends_on': 'Architecture Governance Runbook', 'type': 'REFERENCES'}
- Zależność: {'depends_on': 'Architecture Governance Runbook', 'type': 'REFERENCES'}
