---
title: Cloud Compliance Roadmap
...
## Cel dokumentu
Krótka definicja: Cloud Compliance Roadmap opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
1. Zakres i standardy: ISO 27001/22301, SOC 2, PCI DSS, HIPAA/RODO, NIS2/sector; chmury/projekty w zakresie; model Shared Responsibility.
2. Benchmark i luka: baseline (CIS, cloud provider baseline), ocena luk (gap assessment) względem wymagań.
3. Roadmapa fazowa: etapy (0-baseline, 1-fundamenty, 2-rozszerzenie domen, 3-audit-ready), kamienie milowe i daty.
4. Domeny/kontrolki falami: IAM, sieć/segregacja, data protection (szyfrowanie/klucze), logging/monitoring, SDLC/IaC, DR/BCP, 3rd party, tożsamość maszynowa, sekret/klucz.
5. Automatyzacja i guardrails: IaC/policy-as-code, drift detection, pre-commit/CI, continuous compliance, evidence collection.
6. Dowody i audyty: SoA/controllib, mapowanie requirement→kontrolka→dowód, cykl odświeżania, przygotowanie do audytów zewn./wewn.
7. Metryki i raportowanie: pokrycie kontrolek, drift, czas remediacji, status roadmapy, dashboard.
8. Ryzyka i zależności: vendor/3rd party, zasoby, budżet, zmiany architektury; plan mitigacji.

## Wymagane rozwinięcia
- Benchmark/luka rozwija: CIS Benchmarks, provider baseline, gap assessment report.
- Domeny rozwijają: Cloud Security Baseline (IAM/network/encryption/logging/DR/SDLC/3rd party).
- Automatyzacja rozwija: IaC/policy-as-code/detection-as-code playbooks, Evidence Management/GRC.

## Wymagane streszczenia
- Streszczenie roadmapy (fazy, kluczowe kamienie) i statusu pokrycia kontrolek; top 5 ryzyk/zależności.
...
- DoR:
  - [ ] Zakres chmur/projektów i standardów zdefiniowany; model Shared Responsibility opisany.
  - [ ] Benchmark (CIS/baseline) i wstępny gap assessment dostępne.
  - [ ] Role/ownerzy domen (IAM, sieć, dane, logging, DR, SDLC, 3rd party) przypisane.
- DoD:
  - [ ] Roadmapa fazowa z kamieniami milowymi, domenami i terminami opublikowana.
  - [ ] Mapowanie requirement→kontrolka→dowód przygotowane; metryki/drift/continuous compliance zdefiniowane.
  - [ ] Ryzyka/zależności z planem mitigacji; dokument wersjonowany i udostępniony.
