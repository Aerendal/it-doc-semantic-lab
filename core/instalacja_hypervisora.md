...
Przeprowadzić bezpieczną instalację i bazową konfigurację hypervisora (np. ESXi/KVM/Hyper-V) na serwerach docelowych.
...
1. Wymagania sprzętowe i kompatybilność (CPU/VT, RAM, storage, NIC, HCL).
2. Media i sieć: przygotowanie nośnika, konfiguracja sieci zarządzającej, VLAN/MTU.
3. Instalacja: kroki instalatora, partycjonowanie, sterowniki, pakiety narzędzi.
4. Konfiguracja początkowa: konta, SSH, NTP, logowanie, datastore, wirtualne przełączniki.
5. Bezpieczeństwo i hardening: hasła/klucze, firewall, izolacja mgmt, aktualizacje/patching.
6. Walidacja: test VM, wydajność sieci/storage, monitoring, backup config.
...
- [ ] Sprzęt zgodny z HCL, VT/IOMMU włączone.
- [ ] Sieć zarządzająca skonfigurowana (VLAN/MTU) i dostępna.
- [ ] Hardening podstawowy wykonany (dostępy, firewall, patching).
- [ ] Test VM uruchomiony; monitoring i backup konfiguracji ustawione.
