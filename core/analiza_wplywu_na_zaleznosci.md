---
title: Analiza wpływu na zależności
...
Ocenić, jak planowana zmiana/projekt wpływa na zależności wewnętrzne i zewnętrzne (systemy, zespoły, kontrakty), by zapobiec regresjom i konfliktom.


## Zakres i granice

- Obejmuje: listę zależności (upstream/downstream), wpływ na kontrakty API/formatów, SLA/SLO, harmonogramy innych zespołów, zależności sprzęt/chmura/licencje.
- Nie obejmuje: szczegółowych planów migracji (osobne dokumenty) ani pełnych specyfikacji technicznych zmian.
...
1. Zależności upstream/downstream (co na kogo wpływa).
2. Wpływ na kontrakty i SLA (breaking/non‑breaking, okno migracji).
3. Wpływ na harmonogramy innych zespołów.
4. Ryzyka i mitigacje.
5. Działania koordynacyjne i komunikacja.
...
- [ ] Zależności upstream/downstream zmapowane.
- [ ] Breaking/non‑breaking ocenione; okno migracji ustalone.
- [ ] Koordynacja z zespołami zaplanowana.
- [ ] Ryzyka i mitigacje zapisane.
...
- „Lista zależności” **defines_structure** „Ocena wpływu”.
- „Ryzyka” **feeds** „Działania koordynacyjne”.
