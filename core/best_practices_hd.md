...
Zebrać dobre praktyki dla środowisk high-availability/high-density (np. hosting, bazy, klastry): niezawodność, wydajność, bezpieczeństwo i operacje.


## Zakres i granice

- Obejmuje: architekturę HA (redundancja, quorum), plan capacity, sieć (LB, anycast), storage (replica/erasure), obserwowalność, backup/restore, DR, bezpieczeństwo, procedury operacyjne.
- Nie obejmuje: szczegółów konkretnego produktu (osobne runbooki) ani polityk kosztowych.
...
- Wejścia: SLO/SLA, wymagania obciążenia, architektura usług, ograniczenia infra.
- Wyjścia: guideline HA/HD, checklisty projektowe i operacyjne, wymagania monitoring/alertów, plan DR/backup.
...
1. SLO/SLA i profile obciążenia.
2. Architektura HA (redundancja, quorum, failure domains).
3. Sieć i LB (failover, health checks, anycast/GeoDNS).
4. Storage i dane (replication, consistency, backup/restore).
5. Observability i alerty (metryki, logi, trace).
6. Capacity i scaling (h/v, autoscaling, limits).
7. DR/BCP i testy.
8. Operacje i zmiany (change mgmt, maintenance windows).
...
- [ ] Brak SPOF; quorum/failover opisane.
- [ ] Observability/alerty na SLO.
- [ ] Backup/restore i DR przetestowane.
- [ ] Capacity i limity zaplanowane.
- [ ] Procedury operacyjne i maintenance opisane.


## Definicje robocze

- **SPOF** — single point of failure.
- **Quorum** — mechanizm większości w klastrach.
...
- „Architektura HA” **constrains** „Operacje i DR”.
- „Capacity” **feeds** „Alerty i scaling”.
