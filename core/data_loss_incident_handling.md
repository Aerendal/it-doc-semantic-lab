---
title: Data Loss Incident Handling
...
## Cel dokumentu
Opisuje procedurę postępowania przy incydencie utraty danych (skasowanie, nadpisanie, korupcja), aby szybko ograniczyć wpływ i odzyskać dane.

## Zakres i granice
Obejmuje: identyfikację zakresu utraty, triage (dane krytyczne/PII), decyzję o zatrzymaniu ruchu, ścieżki odzysku (backup, log shipping, wersjonowanie, undelete), komunikację, walidację po odzysku. Nie obejmuje: projektowania backup/DR (osobne dokumenty).

## Wejścia i wyjścia
- Wejścia: zgłoszenie incydentu, logi aplikacyjne/DB, metryki, informacje o backupach/snapshotach, rejestr danych/PII.
- Wyjścia: odzysk danych lub decyzja o częściowym rollbacku, log działań, raport incydentu, plan dalszych mitygacji.
...
- Detekcja i triage (typ danych, krytyczność, PII)
- Decyzja o zatrzymaniu/ograniczeniu ruchu
- Wybór ścieżki odzysku (backup, snapshot, log replay, wersjonowanie)
- Kroki odzysku i weryfikacja integralności
- Komunikacja (wew./zew.) i status
- Dokumentacja incydentu i działania zapobiegawcze
