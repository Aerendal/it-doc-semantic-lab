...
User Stories for Mobile — szablon dokumentu IT.

Zawiera zbiór historyjek użytkownika (user stories) w formacie: Jako [rola] chcę [akcja] po to, żeby [wartość]. Każda story powinna mieć kryteria akceptacji (Given-When-Then), estymację (story points) i powiązanie z epikiem.

> Dokument satelitarny do: "User Stories Mobile" - rozszerza lub uszczegolawia jego tresc.
...
- **Wejścia** (co musi być dostępne przed wypełnieniem): Wizja produktu, persona użytkownika, wyniki badań UX, cele sprintu/releasu, feedback z poprzednich sprintów.
- **Wyjścia** (co dokument wytwarza jako rezultat): Wyspecyfikowane stories gotowe do sprintu (DoR), kryteria akceptacji per story, estymacje, macierz zależności między stories.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): Sprint backlog, plan sprintu, testy akceptacyjne, dokumentacja zmian w produkcie.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Epiki i cel sprintu, persona użytkownika, wyniki testów użyteczności, techniczne spike'i.
