---
title: Design pipeline inferencji
...
Zaprojektować pipeline inferencji ML: serwowanie modeli, skalowanie, bezpieczeństwo i obserwowalność.
...
1. Architektura: online/batch/stream; endpointy; formaty request/response.
2. Model versioning i rollout (canary/blue-green, A/B).
3. Skalowanie i wydajność: autoscaling, batching, cache, latency SLO.
4. Bezpieczeństwo: auth, rate limit, PII, podpisy, isolation.
5. Observability: metrics (latency, error, drift), logs, trace, model monitoring (data/feature drift, performance).
6. CI/CD i testy: contract tests, load tests, shadow mode.
...
- [ ] Versioning/rollout i rollback opisane.
- [ ] Autoscaling/batching/latency SLO zdefiniowane.
- [ ] Security (auth, rate, PII) włączone.
- [ ] Monitoring lat/err/drift i shadow/AB testy ustawione.
