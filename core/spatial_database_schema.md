...
Spatial Database Schema — szablon dokumentu IT.

Opisuje strukturę danych — encje, atrybuty, typy, ograniczenia (constraints) i relacje. Stanowi kontrakt między zespołami (frontend, backend, BI/analytics) i podstawę do migracji schematów.

> Dokument satelitarny do: "Database Schema Design" - rozszerza lub uszczegolawia jego tresc.
...
- **Wejścia** (co musi być dostępne przed wypełnieniem): Wymagania funkcjonalne dot. danych, analiza procesów biznesowych, systemy źródłowe, wymagania GDPR (które pola to dane osobowe).
- **Wyjścia** (co dokument wytwarza jako rezultat): Diagram ERD, słownik danych, skrypty DDL, macierz klasyfikacji danych (sensitivity), plan migracji schematu.
...
- **Wpływa na** (downstream — co zależy od tego dokumentu): Specyfikacje API (request/response schemas), skrypty migracji DB, dokumentacja integracji, polityki retencji, raporty BI.
- **Zależy od** (upstream — co musi istnieć przed tym dokumentem): Wymagania biznesowe, analiza procesów, polityki ochrony danych (GDPR), architektura systemu.
