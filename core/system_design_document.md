...
## Cel dokumentu
Opisać docelowy obraz systemu na poziomie architektury i przepływów, łącząc wymagania biznesowe z projektami technicznymi. Umożliwia spójne decyzje w kolejnych fazach i komunikację ze wszystkimi interesariuszami.

## Zakres i granice
Obejmuje: architekturę wysokiego poziomu, konteksty systemowe, integracje, przepływy danych, domeny/BDD, główne komponenty i ich odpowiedzialności. Nie obejmuje: szczegółowych konfiguracji i kodu (te należą do TDD/Implementation Plan).

## Wejścia i wyjścia
- Wejścia: FRS/NFR, Product Vision, Problem Statement, ADR/RFC, ograniczenia organizacyjne i regulacyjne.
- Wyjścia: zatwierdzony model systemu, zestaw diagramów (kontekst, komponenty, sekwencje), lista integracji, lista decyzji do doprecyzowania w Technical Design.
...
- Konsumuje: wymagania biznesowe i niefunkcjonalne, decyzje z ADR, standardy enterprise.
- Dostarcza do: Technical Design, Data Architecture, API Design, Security Architecture.

## Powiązania sekcja↔sekcja
- Diagram kontekstu → rozwinięcie w integracjach/API.
- Diagram komponentów → rozwinięcie w TDD i kontraktach.
- Przepływy danych → powiązanie z Data Architecture i testami integralności.
...
- Kontekst (zewnętrzne systemy, aktorzy, granice)
- Architektura domenowa / kontekstowa (bounded contexts, główne moduły)
- Architektura logiczna (komponenty, odpowiedzialności)
- Integracje i kontrakty (API/eventy/kolejki)
- Przepływy danych i sekwencje (kluczowe scenariusze end-to-end)
- NFR → wymagania projektowe (wydajność, HA/DR, bezpieczeństwo, zgodność)
- Ryzyka architektoniczne i otwarte decyzje
- Załączniki: diagramy (C4, sekwencje, przepływy)
