...
## Cel dokumentu
Krótka definicja: Cloud Migration Roadmap opisuje cel, rolę w procesie decyzyjnym oraz ryzyka, które dokument ma ograniczać (co najmniej w 2–3 zdaniach).
...
1. Zakres i cele: systemy/projekty w zakresie, KPI/OKR migracji, model docelowy (IaaS/PaaS/SaaS), regiony.
2. Ocena startowa: inwentaryzacja, zależności, readiness (architektura, bezpieczeństwo/compliance, dane, zespół), podział na strumienie.
3. Fazy roadmapy: Faza 0 (baseline/guardrails), Faza 1 (pilot), Faza 2 (skalowanie falami), Faza 3 (optymalizacja/FinOps), z kamieniami milowymi.
4. Domeny i tematy krzyżowe: IAM, sieć/segregacja, dane/szyfrowanie/PII, logi/observability, CI/CD i supply chain, DR/BCP, FinOps, 3rd party.
5. Bezpieczeństwo i compliance: mapowanie na normy (ISO/SOC2/PCI/NIS2/RODO), evidence plan, recertyfikacje, wyjątki/waivery.
6. Plan danych i cutover: strategie (big bang/iteracyjna/hybryda), replika/sync, walidacja integralności, rollback danych.
7. Organizacja i kompetencje: struktura zespołów, odpowiedzialności, szkolenia, kontrakty z vendorami.
8. Harmonogram wysokopoziomowy: fale, zależności, okna zmian, freeze, kryteria wejścia/wyjścia na fazy.
9. Ryzyka i mitigacje: techniczne, organizacyjne, kosztowe; scenariusze awaryjne.
10. Metryki i raportowanie: postęp, jakość (defekty/regresje), bezpieczeństwo/compliance, koszty; dashboardy, rytuały przeglądów.

## Wymagane rozwinięcia
- Ocena startowa i domeny rozwijają: Cloud Security Baseline, Cloud Infrastructure Requirements, Data Migration Plan.
- Bezpieczeństwo/compliance rozwija: Cloud Compliance Roadmap, audit/evidence plan.
- Metryki/FinOps rozwijają: FinOps policy, SLO/SLA, Performance Test Plan.

## Wymagane streszczenia
- Streszczenie faz roadmapy z kamieniami milowymi, kryteriami wejścia/wyjścia i głównymi ryzykami.
...
- DoR:
  - [ ] Zakres systemów/projektów, KPI/OKR migracji i regiony zdefiniowane.
  - [ ] Ocena startowa (architektura, bezpieczeństwo/compliance, dane, zespół) wykonana; podział na strumienie ustalony.
  - [ ] Kamienie milowe i właściciele faz zidentyfikowani; normy do pokrycia określone.
- DoD:
  - [ ] Roadmapa opisuje fazy, domeny (IAM/sieć/dane/logi/CI-CD/DR/FinOps), harmonogram, ryzyka i mitigacje.
  - [ ] Plan danych/cutover i compliance/evidence uwzględnione; kryteria wejścia/wyjścia na fazy zdefiniowane.
  - [ ] Metryki postępu/jakości/bezpieczeństwa/kosztów i rytuały raportowania ustalone; dokument wersjonowany i udostępniony.
