...
Zebrać i uzgodnić wymagania bezpieczeństwa dla systemu/produktu jako wejście do projektowania, implementacji i testów.


## Zakres i granice

- Obejmuje: zakres systemu, klasy danych i ryzyk, wymagania/kontrolki per warstwa, kryteria akceptacji, mapowanie do testów i zgodności.
- Poza zakresem: szczegółowa konfiguracja usług (opis w dokumentach technicznych/runbookach).
...
- Zdefiniuj zakres i klasyfikację danych; spisz wymagania/kontrolki i przypisz właścicieli; sekcje N/A uzasadnij.
- Dodaj quick-links i checklisty DoR/DoD w `reports/checklist_atomic.jsonl`.
- Mapuj wymagania do testów i statusów zgodności; aktualizuj po przeglądach.
...
- Architektura/DFD, mapa danych i wrażliwości.
- Rejestr ryzyk/incydentów, wyniki audytów/testów.


## Wyjścia

- Lista wymagań/kontrolek z właścicielami.
- Kryteria akceptacji i linki do testów.
- Plan przeglądów i status zgodności.
...
- Tabela wymagań: ID → opis → warstwa → kontrolka → właściciel → dowód/test → status.
- Streszczenie kluczowych wymagań i luk.


## Wymagane powiązania

- Polityki, compliance matrix, rejestr ryzyk, architektura/DFD, plan testów.
...
- [ ] Wymagania/kontrolki spisane i przypisane.
- [ ] Kryteria akceptacji/testy podlinkowane.
- [ ] Status/wyjątki wpisane; quick-links/checklisty zaktualizowane.


## Artefakty do załączenia

- Lista wymagań (MD/CSV/JSON).
- Mapa do testów (traceability), diagramy DFD.
- Rejestr wyjątków i plan przeglądów.


## Walidacja / testy

- Peer review wymagań i mapowania do ryzyk/testów.
- Sprawdzenie spójności z architekturą i politykami.


## Metryki monitorowane

- % wymagań pokrytych implementacją/testami.
- Liczba luk/wyjątków i ich wiek.
- Trend zgodności (green/amber/red) per warstwa.


## Utrzymanie i aktualizacje

- Przegląd co release/sprint lub po zmianach architektury/regulacji.
- Aktualizuj wymagania po audytach/incydentach.


## Zakończenie

Po spełnieniu DoD podlinkuj artefakty, odhacz checklisty w `reports/checklist_atomic.jsonl` i zsynchronizuj z planem testów i compliance matrix.
