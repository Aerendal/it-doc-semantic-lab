...
## Cel dokumentu
Przeanalizować incydent bazodanowy (dostępność, opóźnienia, blokady, utrata integralności), ustalić przyczyny i wdrożyć środki zapobiegawcze.

## Zakres i granice
Obejmuje: timeline, wpływ na transakcje/zapytania, blokady/deadlocki, repliki, migracje/DDL, backup/restore, CAPA. Nie obejmuje: pełnego projektu schematu (Database Schema Design).

## Wejścia i wyjścia
- Wejścia: logi DB (slow query, locks), metr. repliki, zmiany DDL/migracje, workload/ruch, alerty SLO, execution plans.
- Wyjścia: postmortem DB, plan mitygacji (indeksy, query tuning, limity, connection pooling), poprawki migracji/rollforward, aktualizacje backup/restore/procedur.
...
- Streszczenie i wpływ (SLO, dane dotknięte, okno)
- Timeline (zmiany, migracje, piki ruchu, awarie repliki)
- Root cause + contributing (locks, plans, storage, konfiguracja)
- Integralność danych i weryfikacje (checksum, audit)
- CAPA: indeksy, zapytania, limity połączeń, konfiguracja repliki/partitioning
- Procedury backup/restore i testy DR – działania korygujące
