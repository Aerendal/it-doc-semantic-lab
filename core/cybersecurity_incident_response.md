...
Opisać proces reagowania na incydenty cyberbezpieczeństwa: klasyfikacja, detekcja, eskalacja, containment, eradication, recovery, komunikacja i post‑incident.


## Zakres i granice

- Obejmuje: klasyfikację/severity, detekcję/alerty, role (SOC/IR/owners), triage, containment/eradication, forensics, komunikację (wewn./zewn.), notyfikacje regulatorów/klientów, recovery, postmortem/CAPA, dowody/audyt.
- Poza zakresem: szczegółowe playbooki specyficzne (ransomware/phishing/DDoS – linkowane), runbooki operacyjne systemów.
...
- Klasyfikacja i severity
- Detekcja i triage
- Containment/eradication/recovery (kroki i właściciele)
- Forensics i dowody
- Komunikacja i notyfikacje
- Postmortem/CAPA
...
- Security Incident Report, Playbooki SOC, Incident Response Plan, Change Mgmt, Legal/Privacy.
...
- Polityka bezpieczeństwa, matryca severity, playbooki SOC, kontakt on-call, logi/alerty, wymagania regulacyjne (RODO/PCI itd.).


## Wyjścia

- Runbook IR, matryca decyzji, checklisty, szablony komunikacji, raport post‑incident z CAPA.



## Jak używać (checklista)

- Określ severity; aktywuj zespół; zbierz dowody.
- Wykonaj containment/eradication; dokumentuj działania; komunikuj status.
- Przywróć usługę; wykonaj postmortem i CAPA; zarchiwizuj dowody.


## Wymagane rozwinięcia / powiązania

- Matryca severity, szablony komunikacji (wew./zewn.), checklisty IR, kontakt on-call, lista regulatorów.


## Kryteria DoR

- Alert/zdarzenie potwierdzone; severity przypisane; zespół dostępny.


## Kryteria DoD

- Incydent zamknięty, usługa przywrócona, postmortem i CAPA zapisane, dowody zarchiwizowane.


## Artefakty

- Raport IR, log działań, dowody/artefakty, CAPA, komunikaty.


## Walidacja

- Przegląd IR; audyt dowodów; test tabletop; weryfikacja CAPA.


## Metryki

- TTD/MTTA/MTTR, liczba incydentów per severity, % CAPA zrealizowanych, notyfikacje w SLA.


## Utrzymanie

- Ćwiczenia IR/tabletop kwartalnie; aktualizacja kontaktów/playbooków; przegląd metryk.


## Zakończenie

Proces IR ogranicza wpływ incydentów; utrzymuj go z aktualnymi playbookami, kontaktami i ćwiczeniami.
