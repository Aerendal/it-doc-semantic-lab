...
Standardy wdrożenia Databricks: workspace, cluster policy, bezpieczeństwo, koszt, CI/CD i governance.
...
1. Workspace i organizacja (folders/repos, secrets, access control).
2. Clusters: polityki (node types, autoscaling, spot vs on‑demand), init scripts, libraries.
3. Storage i dane: mounty, DBFS vs external, katalog/uc, klasyfikacja/PII.
4. Bezpieczeństwo: IAM/SCIM, mTLS, tokeny, audit logs, IP access lists.
5. Koszt: policy na autoscaling/spot, job vs interactive, monitoring kosztów.
6. CI/CD: repos, notebooks jako code, tests, deployment jobs.
7. Governance: Unity Catalog, lineage, metastore, sharing.
...
- [ ] Cluster policies i security (IAM/SCIM, secrets) ustawione.
- [ ] Storage/mounty zgodne z polityką danych.
- [ ] Koszt monitorowany; autoscaling/spot wg zaleceń.
- [ ] CI/CD i testy notebooków/kodu skonfigurowane.
- [ ] Unity Catalog/lineage/metastore używane.
